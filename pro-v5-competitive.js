...
(()=>{
  ...
  function renderTrainingSelect(){
    ...
    const unlocked=chars().filter(c=>gs()?.roster?.includes(c.id)&&(c.id!=='ztaaa'||!!gs()?.ztaaaUnlocked));const first=unlocked[0]?.id||'rojo',dummy=unlocked.find(c=>c.id==='thuvaa')?.id||unlocked[1]?.id||first;
