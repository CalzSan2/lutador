...
(()=>{
  ...
  function ensureSeason(){
    const t=now();if(!data.season.start||!data.season.end||t>=data.season.end){data.season.id=(Number(data.season.id)||0)+1;data.season.start=t;data.season.end=t+SEASON_MS;save();}
  }
  function seasonText(){ensureSeason();const left=Math.max(0,data.season.end-now()),days=Math.ceil(left/86400000);return {days,id:data.season.id}}
  function injectSeasonPanel(){document.querySelector('.v5-season-panel')?.remove()}
  ...
  function renderTrainingSelect(){
    ...
    const owned=chars().filter(c=>gs()?.roster?.includes(c.id)&&(c.id!=='ztaaa'||!!gs()?.ztaaaUnlocked));const unlocked=owned.length?owned:chars().filter(c=>c.id==='rojo').slice(0,1);const first=unlocked[0]?.id||'rojo',dummy=unlocked.find(c=>c.id==='thuvaa')?.id||unlocked[1]?.id||first;
  ...
  function startTraining(p1,p2,dummyMode='standing',stage='dojo'){
    try{startFight(p1,'training',p2,stage);const f=game();if(!f?.p1||!f?.p2)throw new Error('Lutadores indisponíveis');f.v5Training={dummy:dummyMode,totalDamage:0,lastDamage:0,lastHitAt:0,intervalMs:0,hits:0,resetCount:0};f.timer=99999;f.p2.human=false;for(const p of [f.p1,f.p2]){p.maxHp=Math.max(10000000,p.maxHp);p.hp=p.maxHp;p.proStamina=100;p.superMeter=1;p.superReady=true;}document.querySelectorAll('.mega-vs,.v5-vs-pro').forEach(x=>x.remove());f.v5IntroLock=false;f.paused=false;createTrainingHud(f);toast('TREINO INICIADO · R RESETA POSIÇÕES','premium')}catch(err){console.error('[TREINO]',err);toast('Não foi possível iniciar. Volte ao menu e tente novamente.','danger');renderTrainingSelect()}
  }
  ...
  function movesFor(id){let base=[];try{base=(typeof MOVES!=='undefined'&&MOVES[id])?[...MOVES[id]]:[]}catch(_){}const common=[['J / Num1','Poder original'],['↓ + J / ↓ + Num1','Variação rasteira do poder'],['↑ + J / ↑ + Num1','Variação ascendente do poder'],['J no ar / Num1 no ar','Variação aérea do poder'],['H / Num9','Alternar poder, quando disponível'],['Q / Num0','Esquiva perfeita e contra-ataque'],['G / Num4','Gancho Demolidor'],['I / Num5','Soco'],['O / Num6','Chute'],['O no chão','Finalização em rival derrubado']];return [...common,...base,...(EXTRA_MOVES[id]||[])]}
  ...
  /* =========================================================
     ...
     ========================================================= */
  function showProfessionalVs(f){if(!f||f.mode==='training'||f.mode==='tutorial'||f.v5VsShown)return;f.v5VsShown=true;f.v5IntroLock=true;f.paused=true;setTimeout(()=>document.querySelectorAll('.mega-vs').forEach(x=>x.remove()),30);const p=progression(),e=exp();const p1Master=masteryLevel(f.p1.id),p2Master=masteryLevel(f.p2.id);const el=document.createElement('div');el.className='v5-vs-pro';el.innerHTML=`<div class="v5-vs-bg"></div><section class="p1"><img src="ai-assets/characters/${f.p1.id}.png"><div><small>${esc(title())}</small><h2>${esc(f.p1.name)}</h2><span>${esc(title())}</span><b>MAESTRIA ${p1Master} · ${esc(skin())}</b><i>${fmt(e?.stats?.wins||0)} VITÓRIAS · COMBO ${fmt(e?.stats?.maxCombo||0)}x</i></div></section><div class="center"><small>ARENA OFICIAL</small><strong>VS</strong><span>${esc(f.stage?.name||'ARENA')}</span></div><section class="p2"><img src="ai-assets/characters/${f.p2.id}.png"><div><small>${f.p2.human?'RIVAL LOCAL':`CPU · ${(complete()?.aiDifficulty||'normal').toUpperCase()}`}</small><h2>${esc(f.p2.name)}</h2><span>${f.p2.human?'RIVAL LOCAL':'RIVAL DA ARENA'}</span><b>MAESTRIA ${p2Master}</b><i>MELHOR DE 3</i></div></section>`;document.body.appendChild(el);requestAnimationFrame(()=>el.classList.add('show'));setTimeout(()=>{el.classList.add('out');setTimeout(()=>el.remove(),350);if(game()===f){f.v5IntroLock=false;f.paused=false;f.lastTime=performance.now();showRoundBanner(f,1)}},1950)}
  ...
  /* =========================================================
     13. PÓS-LUTA PREMIUM
     ========================================================= */
  function enhanceResult(f,won){setTimeout(()=>{const card=document.querySelector('#match-result-overlay .match-result-card');if(!card||card.querySelector('.v5-result-premium')||!f)return;const s=f.megaStats?.p1||{},xp=progression()?.data?.totalXp||0,master=masteryLevel(f.p1.id),dailyDone=Object.values(data.challenges.daily?.items||{}).filter(x=>x.claimed||x.progress>=1).length;const sec=document.createElement('section');sec.className='v5-result-premium';sec.innerHTML=`<header><small>RELATÓRIO DE CARREIRA</small><h3>${won?'PROGRESSO GARANTIDO':'ANÁLISE DA LUTA'}</h3></header><div class="v5-result-bars"><div><span>XP DA CONTA</span><b>${fmt(xp)} XP</b><i><em style="width:${clamp((xp%1000)/10,4,100)}%"></em></i></div><div><span>MAESTRIA · ${esc(f.p1.name)}</span><b>NÍVEL ${master}</b><i><em style="width:${clamp(((exp()?.mastery?.[f.p1.id]?.xp||0)%260)/2.6,4,100)}%"></em></i></div></div><div class="v5-result-rewards"><span>⚔ ${fmt(s.hits||0)} GOLPES</span><span>🔥 ${fmt(s.maxCombo||0)}x COMBO</span><span>🎯 ${fmt(s.crits||0)} CRÍTICOS</span><span>📋 ${dailyDone} DESAFIOS EM ANDAMENTO</span></div><button class="btn" id="v5-result-challenges">VER DESAFIOS</button>`;card.querySelector('.match-result-actions')?.before(sec);sec.querySelector('#v5-result-challenges').onclick=openChallenges;requestAnimationFrame(()=>sec.classList.add('animate'))},40)}
  ...
  if(typeof baseStartFight==='function')window.startFight=function(){const r=baseStartFight.apply(this,arguments),f=game();if(f){f.v5StartedAt=performance.now();f.v5Round=1;f.v5StartRank=0;for(const p of [f.p1,f.p2])p.v5MinHpRatio=1;if(f.p1?.id==='ytiri')f.p1.rect.w=Math.max(58,f.p1.rect?.w||0);if(f.p2?.id==='ytiri')f.p2.rect.w=Math.max(58,f.p2.rect?.w||0);if(f.mode!=='training'&&f.mode!=='tutorial')setTimeout(()=>showProfessionalVs(f),20)}return r};
  ...
  function injectModes(){const grid=document.querySelector('.modes-screen .mode-grid');if(!grid)return;if(!document.getElementById('mode-v5-training')){const b=document.createElement('button');b.id='mode-v5-training';b.className='mode-card mk-mode v5-mode-training';b.innerHTML='<div class="mode-icon">🎯</div><span class="mode-badge">DANO / FRAMES</span><h3>MODO TREINO</h3><p>Vida e stamina infinitas, dummy e reset rápido.</p>';b.onclick=renderTrainingSelect;grid.appendChild(b)}
    document.querySelector('.v5-season-strip')?.remove();
  }
  ...
  function injectRanking(){document.querySelector('.mega-rank-panel')?.remove();document.querySelector('.v5-season-panel')?.remove()}
