...
(()=>{
  ...
  const VERSION='24.0.0-lendas-da-cronica-loja';
  const SHOP_VERSION=24;
  ...
  const AURORA=Object.freeze({
    id:'aurora',name:'Aurora Valerian',color:'#f2c96d',hp:1980,dmg:34,speed:96,price:1050,
    weapon:'chronoAnchor',special:'brokenEra',portrait:'ai-assets/ravam/legends/aurora-valerian.png',secret:true,ravamOnly:true,ravamLegendExclusive:true
  });
  const KAI_DRACONIS=Object.freeze({
    id:'kai_draconis',name:'Kai Draconis',color:'#b04cff',hp:2200,dmg:37,speed:86,price:1200,
    weapon:'orbitClamp',special:'eventHorizon',portrait:'ai-assets/ravam/legends/kai-draconis.png',secret:true,ravamOnly:true,ravamLegendExclusive:true
  });
  const PRIYA_ARANYA=Object.freeze({
    id:'priya_aranya',name:'Priya Aranya',color:'#73f08f',hp:2040,dmg:31,speed:94,price:1000,
    weapon:'healingSeed',special:'livingSanctuary',portrait:'ai-assets/ravam/legends/priya-aranya.png',secret:true,ravamOnly:true,ravamLegendExclusive:true
  });
  const NUTOA=Object.freeze({
    id:'nutoa',name:'Nutoa Umbra',color:'#d8b4fe',hp:2070,dmg:37,speed:97,price:1250,
    weapon:'eclipseBlades',special:'zeroEclipse',portrait:'ai-assets/ravam/legends/nutoa-umbra.png',secret:true,ravamOnly:true,ravamLegendExclusive:true
  });
  const RIFANA=Object.freeze({
    id:'rifana',name:'Rifana',color:'#5eead4',hp:1960,dmg:35,speed:95,price:1100,
    weapon:'soulThread',special:'soulProcession',portrait:'ai-assets/ravam/legends/rifana.png',secret:true,ravamOnly:true,ravamLegendExclusive:true
  });
  const PETROS=Object.freeze({
    id:'petros',name:'Petros',color:'#43b7ff',hp:1870,dmg:39,speed:210,price:1450,
    weapon:'overclockRush',special:'blindSpot',portrait:'ai-assets/ravam/legends/petros.png',secret:true,ravamOnly:true,ravamLegendExclusive:true
  });
  ...
  const CHRONICLE_IDS=Object.freeze(['aurora','kai_draconis','priya_aranya','nutoa','rifana','petros']);
  const ALL=Object.freeze([PETROS,KAI_DRACONIS,oldPriya,oldKain,oldAria,LEO,THAROTH,AURORA,PRIYA_ARANYA,NUTOA,RIFANA,KTRAK,LARANJA,JONE]);
  const ALL_IDS=Object.freeze(ALL.map(c=>c.id));
  const COSTS=Object.freeze({priya:0,kain:700,aria:650,leo:800,tharoth:900,aurora:1050,kai_draconis:1200,priya_aranya:1000,nutoa:1250,rifana:1100,petros:1450,ktrak:0,laranja:0,jone:0});
  const ABILITY={
    priya:BASE.ability||{flag:'RAVAM',role:'Guardiã da Precisão',tag:'GUARDIÕES DO TEMPO',desc:'Alana Sorelle integra os Guardiões do Tempo. J usa rifle tático; o 3º tiro é explosivo. L ativa uma rajada de 6 disparos.'},
    ...
    leo:Object.freeze({flag:'RAVAM',role:'Guardião dos Portais',tag:'GUARDIÕES DO TEMPO',desc:'Leo Vesper integra os Guardiões do Tempo. J usa pedras através de portais; L cria uma tempestade de portais no céu e nas laterais.'}),
    ...
    aurora:Object.freeze({flag:'RAVAM',role:'Regente do Instante',tag:'GUARDIÕES DO TEMPO',desc:'J marca o rival com um instante que volta para cobrar posição e dano. L quebra uma era, paralisa o tempo e desfere ecos temporais.'}),
    kai_draconis:Object.freeze({flag:'RAVAM',role:'Arquiteto da Gravidade',tag:'GUARDIÕES DO TEMPO',desc:'J cria uma singularidade que puxa e pode ser detonada no segundo comando. L abre um Horizonte de Eventos que comprime e arremessa o rival.'}),
    priya_aranya:Object.freeze({flag:'RAVAM',role:'Guardiã da Cura Viva',tag:'GUARDIÕES DO TEMPO',desc:'J planta uma semente ofensiva que floresce em cura. L ergue um santuário que transforma parte do dano sofrido em recuperação atrasada.'}),
    nutoa:Object.freeze({flag:'RAVAM',role:'Herdeiro do Eclipse',tag:'SOMBRA + LUZ',desc:'Filho de Tharoth Umbra. J alterna lâmina de sombra e lâmina de luz; a luz detona a marca sombria. L faz as duas forças convergirem.'}),
    rifana:Object.freeze({flag:'RAVAM',role:'Tecelã de Almas',tag:'ALMAS',desc:'J captura fragmentos da alma e rouba energia de Super. L convoca um cortejo espectral cujo número de golpes depende das almas reunidas.'}),
    petros:Object.freeze({flag:'RAVAM',role:'Velocista do Ponto Cego',tag:'VELOCIDADE PROIBIDA',desc:'Cada J acelera a Ruptura Fantasma e deixa uma cegueira leve permanente no round. L causa uma cegueira maior e executa o Ponto Cego Supremo.'}),
    ktrak:Object.freeze({flag:'PRIME',role:'Mestre dos Elementos',tag:'AVATAR',desc:'Água nasce do chão, Terra racha o piso, Raio carrega no corpo, Lava abre fendas e Cura ergue água. L ativa o Avatar com transição elemental e versões supremas.'}),
    laranja:Object.freeze({flag:'PRIME',role:'Velocista Supersônico',tag:'SECRETO',desc:'J executa o combo supersônico. L ativa a Cadeia Relâmpago aprimorada.'}),
    jone:Object.freeze({flag:'PRIME',role:'Tático do Veneno',tag:'SECRETO',desc:'J alterna veneno corrosivo, ácido e toxina profunda, acumulando Marca Tóxica. L cria a Praga Tóxica Suprema sem travar o rival.'})
  };
  const META={
    priya:{name:'ALANA SORELLE',tag:'GUARDIÃ DO TEMPO',accent:'#ff765e',portrait:'ai-assets/characters/priya.png',stats:[32,82,1940],moves:[['J','RIFLE · 3º EXPLOSIVO'],['I','SOCO'],['O','CHUTE'],['L','RAJADA TÁTICA · 6 TIROS']]},
    ...
    leo:{name:'LEO VESPER',tag:'GUARDIÃO DO TEMPO',accent:'#67f3b8',portrait:'ai-assets/characters/leo.png',stats:[34,87,2050],moves:[['J','PEDRAS ENTRE PORTAIS'],['I','SOCO'],['O','CHUTE'],['L','COLAPSO DE PORTAIS']]},
    ...
    aurora:{name:'AURORA VALERIAN',tag:'TEMPO',accent:'#f2c96d',portrait:'ai-assets/ravam/legends/aurora-valerian.png',stats:[34,96,1980],moves:[['J','ÂNCORA DO INSTANTE'],['I','SOCO CRONAL'],['O','CHUTE PÊNDULO'],['L','ERA QUEBRADA']]},
    kai_draconis:{name:'KAI DRACONIS',tag:'GRAVIDADE',accent:'#b04cff',portrait:'ai-assets/ravam/legends/kai-draconis.png',stats:[37,86,2200],moves:[['J','ÓRBITA / DETONAR'],['I','SOCO DE MASSA'],['O','CHUTE VETORIAL'],['L','HORIZONTE DE EVENTOS']]},
    priya_aranya:{name:'PRIYA ARANYA',tag:'CURA VIVA',accent:'#73f08f',portrait:'ai-assets/ravam/legends/priya-aranya.png',stats:[31,94,2040],moves:[['J','SEMENTE DE RETORNO'],['I','SOCO RESTAURADOR'],['O','CHUTE VITAL'],['L','SANTUÁRIO DAS TRÊS VIDAS']]},
    nutoa:{name:'NUTOA UMBRA',tag:'SOMBRA + LUZ',accent:'#d8b4fe',portrait:'ai-assets/ravam/legends/nutoa-umbra.png',stats:[37,97,2070],moves:[['J','LÂMINAS DO ECLIPSE'],['I','SOCO SOMBRIO'],['O','CHUTE SOLAR'],['L','ECLIPSE ZERO']]},
    rifana:{name:'RIFANA',tag:'ALMAS',accent:'#5eead4',portrait:'ai-assets/ravam/legends/rifana.png',stats:[35,95,1960],moves:[['J','FIO DA PENITÊNCIA'],['I','SOCO ESPECTRAL'],['O','CHUTE FÚNEBRE'],['L','CORTEJO DOS NOMES']]},
    petros:{name:'PETROS',tag:'MAIS RÁPIDO QUE LARANJA',accent:'#43b7ff',portrait:'ai-assets/ravam/legends/petros.png',stats:[39,540,1870],moves:[['J','RUPTURA FANTASMA'],['I','SOCO VETOR'],['O','CHUTE DE RASTRO'],['L','PONTO CEGO SUPREMO']]},
    ktrak:{name:'KTRAK CALZSAN',tag:'MESTRE DOS ELEMENTOS',accent:'#52d7ff',portrait:'ai-assets/characters/ktrak.png',stats:[36,86,2100],moves:[['J','DOBRA DO ELEMENTO / SUPREMO NO AVATAR'],['H / P2 9','ALTERNAR ELEMENTO'],['I','SOCO'],['O','CHUTE'],['L','MODO AVATAR · TRANSIÇÃO ELEMENTAL']]},
    ...
    jone:{name:'JONE PIKES',tag:'VENENO',accent:'#9bea72',portrait:'ai-assets/characters/jone.png',stats:[34,88,1940],moves:[['J','VENENOS ROTATIVOS + MARCA'],['I','SOCO'],['O','CHUTE'],['L','PRAGA TÓXICA SUPREMA']]}
  };
  ...
  function ensureEconomy(){
    try{
      ...
      const previousStoreVersion=Number(state.ravamStoreVersion||0);
      if(state.ravamModeUnlocked&&previousStoreVersion<17){
      ...
      if(state.ravamModeUnlocked&&previousStoreVersion<SHOP_VERSION){
        // A V23 liberava as Lendas da Crônica junto com o modo. Na V24 elas são produtos da loja.
        const paid=new Set((state.ravamLegendPurchases||[]).filter(x=>Number(x?.cost)>0).map(x=>x.id));
        state.ravamRoster=state.ravamRoster.filter(id=>!CHRONICLE_IDS.includes(id)||paid.has(id));
  ...
  // Primes não existem na interface até o código secreto já ter sido validado.
  const visibleIds=()=>ALL_IDS.filter(id=>!isPrime(id)||isOwned(id));
  const visibleAll=()=>ALL.filter(c=>!isPrime(c.id)||isOwned(c.id));
  const visiblePrimeIds=()=>PRIME_IDS.filter(id=>isOwned(id));
  const hasVisiblePrime=()=>visiblePrimeIds().length>0;
  ...
  function storyRival(id){return ({priya:'aurora',aurora:'kai_draconis',kai_draconis:'priya_aranya',priya_aranya:'leo',leo:'nutoa',nutoa:'tharoth',tharoth:'rifana',rifana:'petros',petros:'aurora',aria:'kain',kain:'leo',ktrak:isOwned('laranja')?'laranja':'priya',laranja:isOwned('jone')?'jone':'ktrak',jone:'priya'})[id]||'kain'}

  // API V17: online enxerga todas as Lendas para resolver nomes, mas a seleção filtra pelo isOwned().
  try{
    const api={...BASE,version:VERSION,cost:ACCESS_COST,priya:oldPriya,kain:oldKain,aria:oldAria,leo:LEO,tharoth:THAROTH,aurora:AURORA,kaiDraconis:KAI_DRACONIS,priyaAranya:PRIYA_ARANYA,nutoa:NUTOA,rifana:RIFANA,petros:PETROS,ktrak:KTRAK,laranja:LARANJA,jone:JONE,isOwned,unlockedIds,displayName};
    delete api.characters;
    delete api.allCharacters;
    Object.defineProperty(api,'characters',{enumerable:true,get:()=>visibleAll()});
    Object.defineProperty(api,'allCharacters',{enumerable:true,get:()=>visibleAll()});
  ...
  function modeCopy(mode){
    ...
    return {eyebrow:'COMBATE SOLO',desc:hasVisiblePrime()?'Escolha sua Lenda ou Prime desbloqueado. A CPU entra com outro personagem R.A.V.A.M.':'Escolha sua Lenda. A CPU entra com outro personagem R.A.V.A.M.'};
  }
  function legendCard(id,selected=false,slot=''){
    const m=META[id],owned=isOwned(id),cost=COSTS[id]||0;
    if(isPrime(id)&&!owned)return '';
  ...
  function renderLocked(){
    ...
    a.innerHTML=`<div class="card mk-card mk-arena ravam-pro-screen r17-screen">
      <section class="ravam-pro-hero locked"><div class="ravam-copy"><small>UNIVERSO PREMIUM</small><h2>R.A.V.A.M <span>STUDIOS</span></h2><p>Adquira o acesso permanente. Aurora, Kai Draconis, Priya Aranya, Nutoa, Rifana e Petros vivem exclusivamente neste modo.</p><div class="ravam-price">🟪 ${fmt(ACCESS_COST)} VANDAIS</div><div class="ravam-wallet-line">SALDO · 🟪 ${fmt(state?.vandais||0)}</div></div><img src="ai-assets/ravam/legends/aurora-valerian.png" alt="Aurora Valerian"></section>
      <section class="ravam-unlock-card"><div><small>ACESSO + 7 LENDAS</small><h3>ALANA + LENDAS DA CRÔNICA</h3><p>As seis novas Lendas vêm liberadas com o modo. Kain, Aria, Leo e Tharoth continuam na Loja R.A.V.A.M.</p></div><button id="r17-buy-access" class="btn big" ${can?'':'disabled'}>${can?'🟪 ADQUIRIR R.A.V.A.M':'VANDAIS INSUFICIENTES'}</button></section>
  ...
  function renderHub(){
    ...
    const primes=visiblePrimeIds();
    const primeNames=primes.map(id=>`<b>${displayName(id)}</b>`).join(' · ');
    const baseNames='GUARDIÕES DO TEMPO · UMBRA · ALMAS · VELOCIDADE';
    const legendCount=ALL_IDS.length-PRIME_IDS.length;
    const countLabel=primes.length?`${legendCount} LENDAS + ${primes.length} PRIME${primes.length>1?'S':''}`:`${legendCount} LENDAS`;
    const showcaseDesc=primes.length?'Tempo, gravidade, cura, luz, sombra, almas e velocidade proibida. Seus Primes secretos também entram no elenco.':'Os Guardiões do Tempo lideram um elenco de Lendas com poderes que não repetem as mecânicas antigas.';
    const primeImgs=primes.slice(0,2).map(id=>`<img src="${META[id].portrait}" alt="${META[id].name}">`).join('');
    a.innerHTML=`<div class="card mk-card mk-arena ravam-pro-screen ravam-v15 r17-screen">
      ...
      <section class="r17-showcase"><div><small>${countLabel} · ${unlockedIds().length} DESBLOQUEADO(S)</small><h3>${baseNames}${primeNames?` · ${primeNames}`:''}</h3><p>${showcaseDesc}</p></div><div class="r17-art-stack r17-art-stack-six"><img src="ai-assets/ravam/legends/aurora-valerian.png" alt="Aurora"><img src="ai-assets/ravam/legends/kai-draconis.png" alt="Kai"><img src="ai-assets/ravam/legends/priya-aranya.png" alt="Priya"><img src="ai-assets/ravam/legends/nutoa-umbra.png" alt="Nutoa">${primeImgs}</div></section>
      ...
      <section class="ravam-battle-grid r17-mode-grid">
        <button id="r17-cpu" class="ravam-battle-card cpu"><span class="ravam-battle-icon">◈</span><small>COMBATE SOLO</small><h3>1P <b>× CPU</b></h3><p>${primes.length?'Escolha sua Lenda ou Prime desbloqueado e enfrente a CPU.':'Escolha sua Lenda e enfrente a CPU.'}</p><em>SELECIONAR →</em></button>
        ...
        <button id="r17-story" class="ravam-battle-card story"><span class="ravam-battle-icon">◇</span><small>CAPÍTULO R.A.V.A.M</small><h3>MODO <b>HISTÓRIA</b></h3><p>Capítulo próprio para cada personagem disponível.</p><em>SELECIONAR →</em></button>
  ...
  function renderShop(){
    ...
    const ids=visibleIds(),withPrime=hasVisiblePrime();
    a.innerHTML=`<div class="card mk-card mk-arena ravam-pro-screen r17-screen r17-shop-screen">
      <section class="ravam-selection-head"><div class="ravam-brand-lockup"><small>COLEÇÃO R.A.V.A.M</small><h2>LOJA DE <span>${withPrime?'LENDAS + PRIMES':'LENDAS'}</span></h2><p>${withPrime?'Compre Lendas e veja seus Primes já desbloqueados.':'Compre cada Lenda uma vez para adicioná-la ao seu elenco.'}</p></div><div class="ravam-head-wallet"><span>SALDO</span><b>🟪 ${fmt(state?.vandais||0)}</b><small>VANDAIS</small></div></section>
      <div class="r17-shop-grid">${ids.map(id=>{const m=META[id],owned=isOwned(id),cost=COSTS[id]||0;return `<article class="r17-shop-card ${id} ${owned?'owned':'locked'}"><div class="r17-shop-art"><img src="${m.portrait}" alt="${m.name}"><span>${owned?(isPrime(id)?'✓ SEU PRIME':'✓ SUA LENDA'):'🔒 BLOQUEADA'}</span></div><div class="r17-shop-body"><small>${m.tag}</small><h3>${m.name}</h3><p>${ABILITY[id].desc}</p><div class="r17-shop-stats"><span><b>${m.stats[0]}</b>DANO</span><span><b>${m.stats[1]}</b>AGIL</span><span><b>${m.stats[2]}</b>VIDA</span></div>${id==='priya'?'<button class="btn" disabled>LENDA INICIAL</button>':owned?`<button class="btn" disabled>✓ ${isPrime(id)?'PRIME DESBLOQUEADO':'LENDA DESBLOQUEADA'}</button>`:`<button class="btn big r17-buy-legend" data-id="${id}" ${Number(state?.vandais||0)>=cost?'':'disabled'}>🟪 COMPRAR · ${fmt(cost)}</button>`}</div></article>`}).join('')}</div>
    ...
    document.querySelectorAll('.r17-buy-legend').forEach(b=>b.onclick=()=>{
      const id=b.dataset.id,cost=COSTS[id]||0;if(isOwned(id)||isPrime(id))return;
  ...
  function renderSelect(mode='cpu'){
    ...
    a.innerHTML=`<div class="card mk-card mk-arena ravam-pro-screen ravam-v15 r17-screen select-mode-${mode}">
      <section class="ravam-selection-head"><div class="ravam-brand-lockup"><small>${copy.eyebrow}</small><h2>SELEÇÃO DE <span>${hasVisiblePrime()?'LENDAS + PRIMES':'LENDAS'}</span></h2><p>${copy.desc}</p></div><button id="r17-change-mode" class="ravam-mini-back">← TROCAR MODO</button></section>
      ...
      <div class="ravam-section-title"><span>02</span><div><small>PASSO DOIS</small><h3>${pickingP2?(hasVisiblePrime()?'P2 · ESCOLHA LENDA / PRIME':'P2 · ESCOLHA LENDA'):(hasVisiblePrime()?'ESCOLHA LENDA / PRIME':'ESCOLHA LENDA')}</h3></div></div>
      <section class="ravam-roster-grid ravam-pick-grid r17-pick-grid">${visibleIds().map(id=>legendCard(id,mode==='pvp'?(p1===id||p2===id):p1===id,mode==='pvp'?(p1===id?'P1':p2===id?'P2':''):(p1===id?'P1':''))).join('')}</section>
      <div class="ravam-selection-help">Lendas com cadeado são adquiridas na Loja R.A.V.A.M. ${mode==='pvp'?'Escolha primeiro P1 e depois P2.':''}</div>
  ...
  function startSelected(mode){
    ...
    try{if(fight){fight.ravamStudios=true;fight.ravamSelectionMode=mode;fight.ravamStory=mode==='story';fight.ravamStoryHero=a;fight.ravamStoryRival=b;fight.ravamStoryTitle=a==='aurora'?'O SEGUNDO QUE NÃO EXISTIU':a==='kai_draconis'?'PESO DE UMA ERA':a==='priya_aranya'?'O JARDIM QUE CURA O TEMPO':a==='nutoa'?'FILHO DO ECLIPSE':a==='rifana'?'OS NOMES SEM CORPO':a==='petros'?'ALÉM DO PONTO CEGO':a==='leo'?'PORTAIS SOBRE O CREPÚSCULO':a==='kain'?'O ESPELHO SEM ROSTO':a==='aria'?'O JARDIM QUE NÃO DORME':a==='tharoth'?'ECO DA SOMBRA IMORTAL':a==='ktrak'?'O EQUILÍBRIO DOS QUATRO':a==='laranja'?'A VELOCIDADE LARANJA':a==='jone'?'NÉVOA VERDE':'A CAÇADORA E O JARDIM'}}catch(_){}
  }
  ...
  if(typeof baseModes==='function')window.renderModes=function(){const r=baseModes.apply(this,arguments);setTimeout(()=>{const card=document.getElementById('mode-ravam');if(!card)return;const p=card.querySelector('p');if(p)p.textContent=hasAccess()?`Loja própria · ${unlockedIds().length} personagem(ns) · 6 novas Lendas exclusivas · Online 2P2.`:'Adquira o modo e receba Aurora, Kai Draconis, Priya Aranya, Nutoa, Rifana e Petros.';const badge=card.querySelector('.mode-badge');if(badge)badge.textContent=hasAccess()?'R.A.V.A.M · LENDAS DA CRÔNICA':`🟪 ${fmt(ACCESS_COST)} VANDAIS`;},0);return r};
  ...
  ensureEconomy();
  window.RavamV17=Object.freeze({version:VERSION,leo:LEO,tharoth:THAROTH,ktrak:KTRAK,renderHub,renderShop,renderSelect,isOwned,get all(){return visibleAll()},syncOnlineUltimate});
})();
