...
(()=>{
  ...
  function rankText(){const d=window.LutadorComplete?.division?.();return d?`${d.icon||'⬟'} ${d.name}`:'⬟ FERRO'}
  ...
  function currentEvent(){const week=Math.floor(Date.now()/(7*86400000))%3;return [
    ...
    {id:'primordial-cup',name:'COPA PRIMORDIAL',color:'#f5cf77',desc:'Qualquer campeão. O foco é vencer e dominar o Ranked.',tasks:[['wins-any','Vença 5 partidas',5,'wins',1000,12],['combo-any','Faça combo de 10+',10,'combo',800,8],['damage-any','Cause 5.000 de dano',5000,'damage',1500,15]]}
  ][week]}
  ...
  /* ---------- LOBBY POLISH ---------- */
  function lobbyPolish(){const menu=document.querySelector('.menu');if(!menu)return;menu.classList.add('v6-lobby');const id=favoriteId();menu.style.setProperty('--v6-lobby-art',`url('ai-assets/characters/${id}.png')`);const actions=menu.querySelector('.lobby-actions');if(actions&&!document.querySelector('.v6-lobby-actions')){const box=document.createElement('div');box.className='v6-lobby-actions';box.innerHTML='<button class="btn" id="v6-profile-btn">👤 PERFIL</button><button class="btn" id="v6-wardrobe-btn">🎨 VESTIÁRIO</button><button class="btn" id="v6-events-btn">⚡ EVENTOS</button>';actions.after(box);box.querySelector('#v6-profile-btn').onclick=openProfile;box.querySelector('#v6-wardrobe-btn').onclick=openWardrobe;box.querySelector('#v6-events-btn').onclick=openEvents}if(!menu.querySelector('.v6-live-strip')){const ev=currentEvent(),strip=document.createElement('section');strip.className='v6-live-strip';strip.innerHTML=`<b>AO VIVO · ${esc(ev.name)}</b><span>${esc(ev.desc)}</span><i>${rankText()} · ${fmt(gs()?.vandais||0)} VANDAIS</i>`;menu.querySelector('.lobby-actions')?.before(strip)}}
