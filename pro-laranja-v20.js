...
(()=>{
  ...
  const VERSION='20.10.8-hyperspeed';
  const ID='laranja';
  ...
  function init(p){
    ...
    if(!Number.isFinite(p.laranjaOverdriveT))p.laranjaOverdriveT=0;
    if(!Number.isFinite(p.laranjaRunPhase))p.laranjaRunPhase=0;
  ...
  function speedCombo(p){
    ...
    p.laranjaCombo={t:0,duration:.40,targetSlot:target.playerSlot,fromX,toX,hitMask:0,lastTrail:0};
    p.throwCd=.46;p.vx=0;setPState(p,'throw');addTrail(f,p,1,.18,.24);hitFx(fromX,body(p),ORANGE,8);sound('attack_light',.80);return true;
  }
  function flashSpecial(p){
    ...
    p.laranjaFlash={
      ...
      doneMask:0,finisher:false,
      // sete passagens: recuo, cinco cortes de velocidade e retorno/impacto final.
      segs:[.070,.052,.048,.048,.048,.052,.070]
    };
    p.laranjaOverdriveT=1.35;
    p.specialCd=Math.max(p.specialCd||0,5.8);setPState(p,'special');
    addBurst(f,p.x,body(p)-12,104,.30,dir);hitFx(p.x,body(p),PALE,12);sound('attack_heavy',.96);toast('LARANJA · RUPTURA HIPERSÔNICA ×3');
    return true;
  }
  ...
  const HIT_TIMES=[.065,.125,.195,.285];
  function stepCombo(f,p,dt){
    ...
    const dashN=smooth(c.t/.065);
    if(target&&target.state!=='ko')c.toX=clamp(target.x-(p.facing||1)*82,28,worldW()-28);
    if(c.t<.085)p.x=lerp(c.fromX,c.toX,dashN);
    else if(target&&target.state!=='ko')p.x=lerp(p.x,clamp(target.x-(p.facing||1)*80,28,worldW()-28),clamp(dt*15,0,1));
    c.lastTrail-=dt;if(c.lastTrail<=0&&c.t<.50){c.lastTrail=.055;addTrail(f,p,c.t<.06?1:2,.10,.14)}
  ...
  function doPhaseHit(f,p,target,phase,dir){
    ...
    // As passagens rápidas causam dano moderado; o impacto final concentra o dano do Super.
    const amounts=[0,.62,.68,.74,.80,.92,0];
    const dmg=(p.dmg||45)*(amounts[phase]||0);
    if(dmg>0){
      applyDamage(target,dmg,p,dir,'laranja-hyperspeed-pass');
      if(target.state!=='ko'){
        target.x=clamp(target.x+dir*(phase>=5?52:24),28,worldW()-28);
        target.vx=dir*(phase>=5?250:110);
      }
      addImpact(f,target.x,body(target)-(phase%2?22:42),phase>=5,phase>=5?PALE:GOLD);
      hitFx(target.x,body(target)-(phase%2?18:40),phase>=5?PALE:ORANGE,phase>=5?12:7);
      sound(phase>=5?'attack_heavy':'attack_light',phase>=5?.82:.50);
    }
  }
  function stepFlash(f,p,dt){
    const s=p.laranjaFlash;if(!s)return;const target=slot(f,s.targetSlot);
    p.state='special';p.stateT=Math.min(p.stateT||0,.42);p.vx=0;p.laranjaMoveAura=.22;
    s.time+=dt;s.lastTrail-=dt;
    const dir=s.dir;
    const targetX=target&&target.state!=='ko'?target.x:s.startX+dir*180;
    const frontX=clamp(targetX-dir*82,36,worldW()-36);
    const backX=clamp(targetX+dir*82,36,worldW()-36);
    const farFront=clamp(targetX-dir*150,36,worldW()-36);
    const farBack=clamp(targetX+dir*150,36,worldW()-36);
    const retreat=clamp(s.startX-dir*145,36,worldW()-36);
    const seq=[
      {from:s.startX,to:retreat,face:dir,hit:false},
      {from:retreat,to:farBack,face:dir,hit:true},
      {from:farBack,to:farFront,face:-dir,hit:true},
      {from:farFront,to:backX,face:dir,hit:true},
      {from:backX,to:frontX,face:-dir,hit:true},
      {from:frontX,to:farBack,face:dir,hit:true},
      {from:farBack,to:s.startX,face:-dir,hit:false}
    ];
    while(s.phase<seq.length){
      const seg=seq[s.phase];
      const distance=Math.max(70,Math.abs(seg.to-seg.from));
      // A duração usa a velocidade real do Laranja (já é 3x a base), então o Super realmente escala com ela.
      const speedBased=clamp(distance/Math.max(1,(p.speed||510)*4.4),.032,.080);
      const dur=Math.min(s.segs[s.phase]||.06,speedBased);
      const n=s.time/dur;
      if(n<1){
        const k=smooth(n);p.x=lerp(seg.from,seg.to,k);p.facing=p.manualFacing=seg.face;
        if(s.lastTrail<=0){s.lastTrail=.038;addTrail(f,p,s.phase%2?2:1,.10,.18)}
        break;
      }
      p.x=seg.to;p.facing=p.manualFacing=seg.face;
      if(seg.hit&&target&&target.state!=='ko')doPhaseHit(f,p,target,s.phase,Math.sign(target.x-p.x)||p.facing||1);
      if(s.phase===0||s.phase===5)addBurst(f,p.x,body(p)-24,s.phase===5?94:68,.18,p.facing);
      s.time-=dur;s.phase++;
      if(s.phase>=seq.length){
        if(!s.finisher&&target&&target.state!=='ko'){
          s.finisher=true;const hitDir=Math.sign(target.x-p.x)||dir||1;
          // Impacto final: toda a velocidade acumulada vira um golpe de ruptura.
          applyDamage(target,(p.dmg||45)*1.85,p,hitDir,'laranja-hyperspeed-finisher');
          target.vx=hitDir*680;target.vy=Math.max(target.vy||0,460);target.onGround=false;target.y=Math.max(14,target.y||0);target.attackDisabledT=Math.max(target.attackDisabledT||0,.34);
          addImpact(f,target.x,body(target)-26,true,'#fffbe8');addBurst(f,target.x,body(target)-24,196,.48,hitDir);hitFx(target.x,body(target)-24,PALE,18);sound('attack_heavy',1);
        }
        p.laranjaOverdriveT=2.4;
  ...
  function updateLaranja(f,dt){
    ...
    for(const p of [f.p1,f.p2])if(p){
      ...
      p.laranjaOverdriveT=Math.max(0,(p.laranjaOverdriveT||0)-dt);
      ...
      if(moving&&p.laranjaTrailCd<=0){p.laranjaTrailCd=(p.laranjaOverdriveT>0?.050:.072);addTrail(f,p,1,.095,p.laranjaOverdriveT>0?.16:(p.onGround?.11:.08))}
    }
  ...
  if(typeof oldStart==='function')window.startFight=function(){const r=oldStart.apply(this,arguments);const f=getFight();if(f){f.laranjaBursts=[];f.laranjaImpacts=[];f.laranjaTrails=[];init(f.p1);init(f.p2);if(f.p1?.id===ID){f.p1.laranjaFlash=f.p1.laranjaCombo=null;f.p1.laranjaOverdriveT=0}if(f.p2?.id===ID){f.p2.laranjaFlash=f.p2.laranjaCombo=null;f.p2.laranjaOverdriveT=0}}return r};
  ...
  function drawWalk(ctx,p){
    const speedRatio=clamp(Math.abs(p.vx||0)/Math.max(1,p.speed||510),.20,1.25);
    const time=performance.now()/1000;
    const cadence=5.4+speedRatio*4.8;
    const a=time*cadence*Math.PI*2;
    const stride=Math.sin(a),contact=Math.abs(Math.cos(a));
    const bob=contact*(2.2+speedRatio*2.0);
    const lean=(.055+.030*speedRatio)+stride*.010;
    const thrust=stride*(2.5+speedRatio*3.5);
    ctx.save();
    ctx.translate(thrust*.20,-bob);
    ctx.rotate(-lean);
    // leve alongamento horizontal reforça aceleração sem aumentar o tamanho vertical.
    ctx.scale(1+speedRatio*.028,1-speedRatio*.010);
    drawPose(ctx,1,1,1);
    ctx.restore();
    // Linhas de velocidade procedurais: não criam partículas nem objetos persistentes.
    ctx.save();ctx.globalCompositeOperation='lighter';ctx.strokeStyle=GOLD;ctx.lineCap='round';
    for(let i=0;i<3;i++){
      const y=-62-i*25+Math.sin(a+i)*3, len=38+speedRatio*42-i*5;
      ctx.globalAlpha=.10+speedRatio*.08-i*.018;ctx.lineWidth=2-i*.35;
      ctx.beginPath();ctx.moveTo(-24-i*7,y);ctx.lineTo(-24-len,y+stride*2);ctx.stroke();
    }
    ctx.restore();
  }
  function drawFlash(ctx,p){const s=p.laranjaFlash,ph=s?.phase||0;ctx.save();ctx.translate(0,-3);ctx.rotate(-(p.facing||1)*(ph===0?.07:.045));ctx.scale(1.045,.985);drawPose(ctx,ph===0?1:(ph%2?2:5),1,1);ctx.restore();ctx.save();ctx.globalCompositeOperation='lighter';ctx.strokeStyle=PALE;ctx.lineWidth=3;ctx.globalAlpha=.22;for(let i=0;i<3;i++){const y=-52-i*28;ctx.beginPath();ctx.moveTo(-22,y);ctx.lineTo(-92-i*18,y+(i-1)*3);ctx.stroke()}ctx.restore()}
  function drawLaranjaFighter(ctx,p){
    ...
    if((p.laranjaOverdriveT||0)>0){const a=clamp(p.laranjaOverdriveT/2.4,0,1);ctx.save();ctx.translate(p.x,floor);ctx.globalCompositeOperation='lighter';ctx.strokeStyle=ORANGE;ctx.lineWidth=2.2;for(let i=0;i<3;i++){ctx.globalAlpha=(.12+i*.035)*a;ctx.beginPath();ctx.ellipse(0,-88,42+i*9,82+i*7,0,0,Math.PI*2);ctx.stroke()}ctx.restore()}
    return true;
  }
