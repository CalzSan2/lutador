/* SORTEP NIAK — AURORA + PETROS + KAI DRACONIS · ANIMAÇÃO V27
 * Integra Aurora Valerian, Petros e Kai Draconis ao mesmo motor visual dos campeões (ProMotion).
 * Usa atlas transparentes limpos, corrige cortes do Kai, melhora o recorte do Petros e aplica
 * passos por distância, idle, guarda, ataques, poderes e especial com a mesma mecânica visual.
 * Mantém 100% das regras, golpes e poderes das Lendas da Crônica V24.
 */
(()=>{
  'use strict';
  if(window.RavamLegendChampionAnimationV27?.version)return;
  const VERSION='27.0.0-ravam-champion-motion';
  const COLS=6;
  const ROOT='ai-assets/ravam/legends/animation-pro/';
  const CONFIG=Object.freeze({
    aurora:{file:'aurora-pro-atlas.png',height:238,accent:'#f2c96d',cacheId:'v27-aurora'},
    petros:{file:'petros-pro-atlas.png',height:226,accent:'#43b7ff',cacheId:'v27-petros'},
    kai_draconis:{file:'kai_draconis-pro-atlas.png',height:226,accent:'#b04cff',cacheId:'v27-kai-draconis'}
  });
  ...
  function motionFor(p){
    ...
    const proxy=Object.assign({},p,{walkDistance:dist});
    return window.ProMotion.motionFor(proxy);
  }
  ...
  function drawGuard(ctx,p,floor,height){
    ...
    ctx.lineWidth=2; ctx.globalAlpha=.5; ctx.beginPath();
    ctx.moveTo(8,-height*.39); ctx.quadraticCurveTo(height*.47,-height*.23,height*.42,height*.18);
    ctx.quadraticCurveTo(height*.16,height*.34,-5,height*.14); ctx.closePath(); ctx.stroke();
    ctx.globalAlpha=.07; ctx.fill(); ctx.restore();
  }
  function drawAttackFx(ctx,p,floor,height,motion){
    ...
    if(p.id==='aurora'){
      ctx.lineWidth=3;
      ctx.beginPath(); ctx.arc(0,0,height*(.30+.06*impact),-.55,2.3); ctx.stroke();
  ...
  function drawHitFlash(ctx,p,floor,height){
    if((p.hitFlash||0)<=.035)return;
    ctx.save(); ctx.strokeStyle='#fff'; ctx.lineWidth=2; ctx.globalAlpha=Math.min(.8,(p.hitFlash||0)*3);
  ...
  function drawChampionLegend(ctx,p){
  ...
  if(typeof previousDraw==='function') window.drawFighter=function(ctx,p){
    if((p?.id==='aurora'||p?.id==='petros'||p?.id==='kai_draconis')&&drawChampionLegend(ctx,p)) return;
  ...
  if(typeof oldStart==='function'&&!oldStart.__ravamLegendV27){
    ...
    wrapped.__ravamLegendV27=true; window.startFight=wrapped;
  }
  ...
  window.RavamLegendChampionAnimationV27=Object.freeze({version:VERSION,ids:['aurora','petros','kai_draconis'],engine:'ProMotion',cycles:window.ProMotion?.cycles||null});
})();
