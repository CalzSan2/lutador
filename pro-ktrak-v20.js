/* SORTEP NIAK — KTRAK CALZSAN — MESTRE DOS ELEMENTOS
 * PRIME secreto. J usa o elemento atual; H / P2 9 alterna os elementos.
 * L ativa o Modo Avatar, com versões supremas das dobras.
 */
(()=>{
  'use strict';
  if(window.KtrakV20?.version)return;
  const VERSION='20.10.9-element-master';
  ...
  function ensure(f){
    if(!f)return;
    for(const k of ['ktrakShots','ktrakRocks','ktrakBursts','ktrakFx','ktrakTornadoes','ktrakInfernos','ktrakEarthCrushes','ktrakLavaPools','ktrakHealColumns','ktrakLightningCharges','ktrakGroundCracks'])if(!Array.isArray(f[k]))f[k]=[];
  }
  function initFighter(p){
    ...
    if(!Number.isFinite(p.ktrakAvatarIntroT))p.ktrakAvatarIntroT=0;
    if(!Number.isFinite(p.ktrakLightningChargeT))p.ktrakLightningChargeT=0;
  }
  ...
  function groundCrack(f,x,size=72,life=4.8){
    if(!f)return;ensure(f);if(f.ktrakGroundCracks.length>=6)f.ktrakGroundCracks.shift();
    f.ktrakGroundCracks.push({x:clamp(x,34,W-34),size,life,maxLife:life,seed:Math.random()*1000});
  }
  function queueLightning(f,p,t,dir,avatar=false){
    ensure(f);if(f.ktrakLightningCharges.length>=2)f.ktrakLightningCharges.shift();
    const charge=avatar?.34:.26;
    p.ktrakLightningChargeT=charge;p.ktrakCastT=Math.max(p.ktrakCastT||0,charge+.10);
    f.ktrakLightningCharges.push({ownerSlot:p.playerSlot,targetSlot:t.playerSlot,dir,life:charge,maxLife:charge,avatar,dead:false});
    textFx(f,p,avatar?'AVATAR · CONDUÇÃO DO RAIO':'RAIO · CONDUZINDO ENERGIA',COLORS.lightning,.48);
  }
  ...
  function avatarElementalAttack(p){
    ...
    if(type==='water'){
      ...
      f.ktrakShots.push({kind:'water',avatarSupreme:true,groundBorn:true,emergeT:.22,emergeMax:.22,startX,x:startX,y:GROUND_Y+24,baseY:GROUND_Y-62,vx:dir*940,life:2.0,maxLife:2.0,ownerSlot:p.playerSlot,targetSlot:t.playerSlot,dmg:(p.dmg||36)*1.36,dead:false,r:52,phase:Math.random()*6.28,trail:[],dir,waveHeight:142});
      ...
      groundCrack(f,center-gap,92,5.4);groundCrack(f,center+gap,92,5.4);groundCrack(f,center,70,4.6);textFx(f,p,'AVATAR · CORREDOR TITÂNICO',COLORS.earth,.96);sparkFx(center-gap,GROUND_Y-8,COLORS.earth,34);sparkFx(center+gap,GROUND_Y-8,COLORS.earth,34);p.throwCd=.88;
    }else if(type==='lightning'){
      queueLightning(f,p,t,dir,true);
      p.throwCd=.66;
    }else if(type==='lava'){
  ...
  function elementalAttack(p){
    ...
    if(type==='earth'){
      ...
      groundCrack(f,startX,78,4.8);textFx(f,p,'TERRA · ROCHA ERGUIDA DO SOLO',COLORS.earth,.78);sparkFx(startX,GROUND_Y-7,COLORS.earth,16);sound('attack_heavy',.72);
    }else if(type==='lava'){
      if(f.ktrakLavaPools.length>=2)f.ktrakLavaPools.shift();
  ...
  function avatarMode(p){
    const f=getFight();if(!f||!p||p.state==='ko')return;ensure(f);initFighter(p);
    p.ktrakAvatarT=8;p.ktrakAvatarMax=8;p.ktrakAvatarIntroT=.82;p.ktrakCastT=.82;p.specialCd=7.2;setPState(p,'special');
  ...
  function updateKtrak(f,dt){
    ...
    for(const p of [f.p1,f.p2])if(p){
      if(p.id==='ktrak'){
        initFighter(p);p.ktrakSwitchCd=Math.max(0,(p.ktrakSwitchCd||0)-dt);p.ktrakCastT=Math.max(0,(p.ktrakCastT||0)-dt);p.ktrakAvatarIntroT=Math.max(0,(p.ktrakAvatarIntroT||0)-dt);p.ktrakLightningChargeT=Math.max(0,(p.ktrakLightningChargeT||0)-dt);
    ...
    for(const q of f.ktrakLightningCharges){
      if(q.dead)continue;q.life-=dt;
      const owner=slotOf(f,q.ownerSlot),target=slotOf(f,q.targetSlot);
      if(!owner||owner.state==='ko'||!target||target.state==='ko'){q.dead=true;continue;}
      owner.facing=q.dir||owner.facing||1;
      if(q.life<=0){
        q.dead=true;
        if(f.ktrakShots.length>=6)f.ktrakShots.shift();
        const startX=clamp(owner.x+(q.dir||1)*64,36,W-36),targetX=clamp(target.x+(q.dir||1)*18,42,W-42);
        if(q.avatar){
          f.ktrakShots.push({kind:'lightning',avatarSupreme:true,beam:true,anchorToOwner:true,x:startX,y:GROUND_Y-118,baseY:GROUND_Y-118,vx:0,life:.58,maxLife:.58,ownerSlot:q.ownerSlot,targetSlot:q.targetSlot,dmg:(owner.dmg||36)*.42,dead:false,r:26,phase:Math.random()*6.28,dir:q.dir,length:Math.max(220,Math.min(620,Math.abs(targetX-startX)+96)),tick:.05,hitTick:0,hitCount:0});
          textFx(f,owner,'AVATAR · RAIO AZUL CONTÍNUO',COLORS.lightning,.70);
        }else{
          f.ktrakShots.push({kind:'lightning',beam:true,avatarSupreme:false,x:startX,y:GROUND_Y-118,baseY:GROUND_Y-118,vx:0,life:.24,maxLife:.24,ownerSlot:q.ownerSlot,targetSlot:q.targetSlot,dmg:(owner.dmg||36)*1.10,dead:false,r:18,phase:Math.random()*6.28,dir:q.dir,length:Math.max(200,Math.min(520,Math.abs(targetX-startX)+72)),hasHit:false});
        }
        sparkFx(startX,GROUND_Y-92,COLORS.lightning,12);sound('attack_light',.88);
      }
    }
    f.ktrakLightningCharges=f.ktrakLightningCharges.filter(q=>!q.dead);
      ...
      if(r.mode==='ground-launch'){
        ...
        if(r.phaseName==='rise'){
          ...
          if(target&&target.state!=='ko'&&Math.abs(r.x-target.x)<86&&hitY(target,r.y,56)){
            ...
            groundCrack(f,r.x,68,4.2);burst(f,'earth-impact',r.x,body(target),COLORS.earth,.64,104,dir);sparkFx(r.x,body(target),COLORS.earth,16);textFx(f,target,'ROCHA DO SOLO',COLORS.earth,.60);
          }
    ...
    for(const cr of f.ktrakGroundCracks)cr.life-=dt;
    f.ktrakGroundCracks=f.ktrakGroundCracks.filter(cr=>cr.life>0);
  ...
  function drawKtrakFighter(ctx,p){
    ...
    if(downed){const dimg=POSES[3]||img;ctx.translate(-10,-10);ctx.rotate(-Math.PI/2);ctx.translate(0,70);drawRawPose(ctx,dimg,194,.92)}
      ...
      if((p.ktrakLightningChargeT||0)>0){const q=clamp(p.ktrakLightningChargeT/.34,0,1);ctx.translate(7*(1-q),-2);ctx.rotate(-.035*(1-q));}
  ...
  function drawShot(ctx,z){
    const col=COLORS[z.kind]||'#fff',dir=z.dir||((z.vx||1)>=0?1:-1);ctx.save();ctx.translate(z.x,z.y);ctx.scale(dir,1);ctx.globalAlpha=.92;
    if(z.kind==='water'){
      if((z.emergeT||0)>0){
        ...
        ctx.fillStyle='#42c9ef';ctx.beginPath();ctx.moveTo(-68,20);ctx.quadraticCurveTo(-48,-12,-18,-42);ctx.quadraticCurveTo(4,-h*.99,28,-62);ctx.quadraticCurveTo(50,-30,70,16);ctx.quadraticCurveTo(24,8,-68,20);ctx.fill();ctx.globalAlpha=.32;ctx.fillStyle='#bff8ff';ctx.beginPath();ctx.moveTo(-56,14);ctx.quadraticCurveTo(-28,-22,0,-46);ctx.quadraticCurveTo(26,-72,53,8);ctx.closePath();ctx.fill();ctx.globalAlpha=.92;
        ...
        ctx.strokeStyle='#f5ffff';ctx.lineWidth=2.5;ctx.beginPath();ctx.moveTo(-18,-32);ctx.quadraticCurveTo(8,-h*.95,36,-43);ctx.stroke();ctx.globalAlpha=.72;for(let i=0;i<4;i++){ctx.beginPath();ctx.arc(-26+i*18,-22-Math.abs(Math.sin((z.phase||0)+i))*18,2.5+(i%2),0,Math.PI*2);ctx.stroke();}
      }
    }else if(z.kind==='fire'){
  ...
  function drawGroundCrack(ctx,cr){
    const a=clamp(cr.life/(cr.maxLife||1),0,1),sz=cr.size||72,seed=cr.seed||0;ctx.save();ctx.translate(cr.x,GROUND_Y+1);ctx.globalAlpha=.62*Math.min(1,a*2);ctx.strokeStyle='rgba(91,57,34,.88)';ctx.lineWidth=3;ctx.lineCap='round';
    for(let i=-2;i<=2;i++){const x=i*sz*.13+Math.sin(seed+i)*5;ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x+i*4,-8-Math.abs(i)*2);ctx.lineTo(x+i*9,-2-Math.abs(Math.sin(seed+i))*8);ctx.stroke();}
    ctx.strokeStyle='rgba(221,171,103,.42)';ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(-sz*.44,-1);ctx.lineTo(-sz*.18,-8);ctx.lineTo(0,-3);ctx.lineTo(sz*.22,-10);ctx.lineTo(sz*.45,-1);ctx.stroke();ctx.restore();
  }
  function drawLightningCharge(ctx,q,f){
    const owner=slotOf(f,q.ownerSlot);if(!owner)return;const n=1-clamp(q.life/(q.maxLife||1),0,1),tm=performance.now()/1000,dir=q.dir||owner.facing||1,y=body(owner)-66;
    ctx.save();ctx.translate(owner.x,y);ctx.scale(dir,1);ctx.globalCompositeOperation='lighter';ctx.lineCap='round';
    for(let i=0;i<3;i++){ctx.globalAlpha=.34+.18*n;ctx.strokeStyle=i===2?'#f8feff':'#5ab7ff';ctx.lineWidth=i===2?2:3;ctx.beginPath();const sy=-18+i*18;ctx.moveTo(-22,sy);ctx.lineTo(-8+Math.sin(tm*16+i)*5,sy-12);ctx.lineTo(8+Math.cos(tm*14+i)*5,sy+7);ctx.lineTo(28,sy-5);ctx.stroke();}
    ctx.globalAlpha=.20+.24*n;ctx.strokeStyle='#dff9ff';ctx.lineWidth=2;ctx.beginPath();ctx.arc(16,-4,22+10*n,0,Math.PI*2);ctx.stroke();ctx.restore();
  }
  function drawAvatarIntro(ctx,p){
    if((p.ktrakAvatarIntroT||0)<=0)return;const n=1-clamp(p.ktrakAvatarIntroT/.82,0,1),tm=performance.now()/1000,y=body(p)-58;ctx.save();ctx.translate(p.x,y);ctx.globalCompositeOperation='lighter';ctx.globalAlpha=(1-n)*.54+.18;
    const cols=['#4bdcff','#d7a85f','#ff6a2d','#d8f7ff'];for(let i=0;i<4;i++){const a=tm*3.2+i*Math.PI/2,r=38+34*n,x=Math.cos(a)*r,yy=Math.sin(a)*r*.55;ctx.fillStyle=cols[i];ctx.beginPath();ctx.arc(x,yy,5+3*(1-n),0,Math.PI*2);ctx.fill();}
    ctx.strokeStyle='#bff6ff';ctx.lineWidth=3;ctx.globalAlpha=.42*(1-n);ctx.beginPath();ctx.arc(0,0,34+68*n,0,Math.PI*2);ctx.stroke();ctx.restore();
  }
  ...
  function drawKtrakFx(ctx,f){
    if(!ctx||!f)return;
    for(const cr of (f.ktrakGroundCracks||[]))drawGroundCrack(ctx,cr);
    for(const q of (f.ktrakLightningCharges||[]))drawLightningCharge(ctx,q,f);
