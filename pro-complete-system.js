...
(()=>{
  ...
  function showUltimate(p){
    ...
    const f=game();
    const committed=p.v32Ultimate;
    if(f){initStats(f);const st=fightStat(p);if(st&&!committed)st.ultimates++;f.megaHitStop=Math.max(f.megaHitStop||0,.94);f.v6UltimateOwner=p;}
    const ULTIMATE_NAMES={
      ...
      aurora:['ERA QUEBRADA','O SEGUNDO QUE NÃO EXISTIU'],
      kai_draconis:['HORIZONTE DE EVENTOS','PESO DE UMA ERA'],
      priya_aranya:['SANTUÁRIO DAS TRÊS VIDAS','JARDIM DO RETORNO'],
      nutoa:['ECLIPSE ZERO','CONVERGÊNCIA UMBRA'],
      rifana:['CORTEJO DOS NOMES','PROCISSÃO DAS ALMAS'],
      petros:['PONTO CEGO SUPREMO','RUPTURA HIPERVELOZ']
    };
    const names=ULTIMATE_NAMES[p.id]||[String(p.name||'CAMPEÃO').toUpperCase(),'ULTIMATE SUPREMA'];
    const ULTIMATE_PORTRAITS={
      aurora:'ai-assets/ravam/legends/aurora-valerian.png',
      kai_draconis:'ai-assets/ravam/legends/kai-draconis.png',
      priya_aranya:'ai-assets/ravam/legends/priya-aranya.png',
      nutoa:'ai-assets/ravam/legends/nutoa-umbra.png',
      rifana:'ai-assets/ravam/legends/rifana.png',
      petros:'ai-assets/ravam/legends/petros.png'
    };
    const portrait=ULTIMATE_PORTRAITS[p.id]||`ai-assets/characters/${p.id}.png`;
    const el=document.createElement('div');el.className='mega-ultimate-cine '+(p.playerSlot==='p2'?'right':'left');
    el.innerHTML=`<img src="${portrait}" alt="${p.name}" onerror="this.style.opacity=.12;this.removeAttribute('src')"><div><small>ULTIMATE CINEMATOGRÁFICA</small><h1>${names[0]}</h1><h2>${names[1]}</h2><span>${p.name}</span><em>O GOLPE SÓ É LIBERADO QUANDO ESTA TELA TERMINAR</em></div>`;
    document.body.appendChild(el);requestAnimationFrame(()=>el.classList.add('show'));
    setTimeout(()=>{
      el.classList.remove('show');setTimeout(()=>el.remove(),180);
      // A delayed callback from a previous round must never arm a new Super.
      if(committed&&(p.v32Ultimate!==committed||game()!==f||f.niakV32Epoch!==committed.epoch))return;
      p.megaUltimateCineLock=false;p.megaUltimateCineDone=true;
    },820);
  ...
  if(typeof oldPlayerInput==='function')window.playerInput=function(p){
    ...
    const localV32=!!window.NiakV32Combat?.supports?.(f);
    if(localV32&&p.v32Ultimate){
      const tx=p.v32Ultimate;
      window.justPressed=window.justPressed||{};window.justPressed[superKey]=false;
      if(tx.fight!==f||tx.epoch!==f.niakV32Epoch||f.over||p.hp<=0||p.state==='ko'){
        window.NiakV32Combat.cancelUltimate(p);return;
      }
      if(f.paused||!p.megaUltimateCineDone)return;
      // Commit once, then use precisely the same character dispatch as the core.
      // Re-injecting L used to lose the attack inside frozen()/proMove guards.
      p.v32Ultimate=null;p.megaUltimatePending=false;p.megaUltimateCineDone=false;p.megaUltimateCineLock=false;
      p.megaUltimateBypass=false;p.attackDisabledT=0;p.specialCd=0;p.defend=false;p.proMove=null;p.proBuffer=null;
      p.state=p.onGround?'idle':'air';p.stateT=0;
      if(!p.superReady){p.proStamina=Math.min(100,(p.proStamina??0)+tx.cost);return;}
      consumeSuper(p);p.megaUltimateBoostT=1.6;
      const st=fightStat(p);if(st)st.ultimates++;
      if(p.isKlopp)kloppSuper(p);
      else if(f.mode==='mix')mixedSuper(p);
      else if(p.id==='frogh')froghTransform(p);
      else if(p.id==='uiye'&&p.uiyeCopyT>0)uiyeUseCopiedSpecial(p);
      else if(p.id==='uiye')uiyeCopyPower(p);
      else if(p.id==='ztaaa')bossSuper(p);
      else if(p.id==='nine85')nine85SystemCrash(p);
      else castSpecial(p);
      if(typeof charSound==='function')charSound(p,'special');
      window.NiakV32Combat.record('ultimate',p,{name:p.name||p.id});
      return;
    }
    ...
    if(p.megaUltimatePending){
      ...
      window.justPressed[superKey]=false;
      if(localV32&&(f.over||f.paused||p.state==='ko'||p.state==='hurt'||p.freezeT>0||p.proKnockdownT>0||p.kloTrap||p.proMove))return;
      if(p.proStamina!==undefined&&p.proStamina<28){toast('ULTIMATE EXIGE 28 DE STAMINA','danger');return;}
      if(localV32)p.v32Ultimate={fight:f,epoch:f.niakV32Epoch,cost:p.proStamina===undefined?0:28};
      ...
      if(!showUltimate(p)){p.megaUltimatePending=false;if(localV32)window.NiakV32Combat.cancelUltimate(p);}
      return;
    }
  ...
  if(typeof oldModes==='function')window.renderModes=function(){rankedPending=false;const r=oldModes.apply(this,arguments);document.getElementById('mode-ranked')?.remove();difficultyPanel();return r};
