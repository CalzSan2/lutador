...
(()=>{
  ...
  const ACH=[
    ...
    {id:'primordial',name:'Ascensão Primordial',desc:'Chegue ao rank Primordial.',test:()=>Number(window.LutadorComplete?.meta?.rank?.rp||0)>=2500,reward:{vandais:50,title:'Ascendente Primordial'}},
  ...
  function profileSnapshot(){
    const pp=window.ProProgression?.getSnapshot?.();const rank=window.LutadorComplete?.division?.()||{name:'FERRO',icon:'⬟'};let fav='—',best=-1;
    for(const [id,s] of Object.entries(data.stats.byFighter)){if((s.matches||0)>best){best=s.matches||0;fav=charBy(id)?.name||id}}
    return {level:pp?.profile?.level||1,xp:pp?.data?.totalXp||0,title:pp?.profile?.displayTitle||'NOVO DESAFIANTE',rank:`${rank.icon||''} ${rank.name||'FERRO'}`,fav};
  }
