...
(()=>{
  ...
  function impact(f,x,y,color,blocked=false){
    if(f.proImpacts.length>=14)f.proImpacts.shift();
  ...
  function step(f,dt){
    ...
    f.proProjectiles=f.proProjectiles.filter(s=>!s.dead).slice(-8);
  ...
  function drawCombat(c,f){
    ...
    for(const e of f.proImpacts){
      ...
      for(let i=0;i<4;i++){const a=i*Math.PI*2/4,r=12+t*36;c.beginPath();c.moveTo(Math.cos(a)*r*.5,Math.sin(a)*r*.5);c.lineTo(Math.cos(a)*r,Math.sin(a)*r);c.stroke()}
