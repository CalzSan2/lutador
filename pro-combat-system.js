...
(()=>{
  ...
  function openGuide(training=false){
    ...
    inputDialog.querySelector('#pro-training-start').onclick=()=>{const id=inputDialog.querySelector('select').value;inputDialog.close();startTraining(id)};
    inputDialog.showModal();if(training)inputDialog.querySelector('select').focus();
  }
  function addButtons(){
    const controlBox=document.querySelector('.control-box');
    if(controlBox&&!controlBox.dataset.combatControls){controlBox.dataset.combatControls='1';controlBox.textContent='P1: A/D mover · W pular · J poder · I soco · O chute · G gancho · K defesa · L super.  P2: setas · Num 1 poder · Num 5 soco · Num 6 chute · Num 4 gancho · Num 2 defesa · Num 3 super.  F1: combinações e treino.'}
    const actions=document.querySelector('.pro-legacy-actions');
    if(actions&&!actions.querySelector('[data-combat-guide]')){const b=document.createElement('button');b.className='btn';b.dataset.combatGuide='1';b.textContent='🥊 GOLPES & TREINO';b.onclick=()=>openGuide();actions.appendChild(b)}
    const modes=document.querySelector('.mode-grid');
    if(modes&&!modes.querySelector('[data-combat-training]')){const b=document.createElement('button');b.className='mode-card mk-mode';b.dataset.combatTraining='1';b.innerHTML='<span class="mode-icon">🥊</span><h3>TREINO LIVRE</h3><p>Teste todos os campeões, socos, chutes e técnicas. Sem gastar moedas.</p>';b.onclick=()=>openGuide(true);modes.appendChild(b)}
    const pause=document.querySelector('.pause-grid');
    if(pause&&!pause.querySelector('[data-combat-guide]')){const b=document.createElement('button');b.className='pause-btn';b.dataset.combatGuide='1';b.textContent='🥊 COMANDOS E TREINO';b.onclick=()=>openGuide();pause.appendChild(b)}
  }
  let pending=false;
