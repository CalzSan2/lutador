...
(()=>{
  ...
  const CONFIG=Object.freeze({
    ...
    rifana:{file:'rifana-pro-atlas.png',height:320,accent:'#5eead4',cacheId:'v31-rifana'}
  });
  ...
  function drawGuard(ctx,p,floor,height){
    ...
    const accent=CONFIG[p.id]?.accent||p.color||'#67e8f9';
    ctx.strokeStyle=p.id==='aurora'?'#fff6d8':(p.id==='rifana'?'#d9fffb':'#eafcff');ctx.fillStyle=accent;
  ...
  function drawAttackFx(ctx,p,floor,height,motion){
    ...
    if(p.id==='aurora'){
      ...
      ctx.lineWidth=4;
      ctx.beginPath();ctx.arc(height*.08,-height*.02,height*(.18+.03*impact),0,Math.PI*2);ctx.stroke();
      ctx.beginPath();ctx.arc(height*.26,-height*.10,height*(.10+.02*impact),0,Math.PI*2);ctx.stroke();
      ctx.beginPath();ctx.moveTo(-height*.08,height*.05);ctx.quadraticCurveTo(height*.18,-height*.20,height*(.52+.12*impact),-height*.06);ctx.stroke();
