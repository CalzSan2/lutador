/*
 * LUTADOR V22 — CONFRONTO DA ILHA
 * Platform fighter para 2–8 jogadores, treino com bots e P2P host-autoritativo.
 */
(()=>{
'use strict';
const VERSION='22.8.0',CW=1440,CH=810,WORLD_W=CW*3,FALL_Y=930;
...
const FORMAT={ffa:{id:'ffa',name:'TODOS CONTRA TODOS',short:'FFA',min:2,max:8,teams:0},'2v2':{id:'2v2',name:'2 × 2',short:'2V2',min:4,max:4,teams:2},'3v3':{id:'3v3',name:'3 × 3',short:'3V3',min:6,max:6,teams:2},'4v4':{id:'4v4',name:'4 × 4',short:'4V4',min:8,max:8,teams:2},duo:{id:'duo',name:'EVENTO EM DUPLAS',short:'DUPLAS',min:2,max:2,teams:1}};
...
// V22.1: a arena tem 3 telas completas de largura. Os lutadores começam no setor central
// e a câmera acompanha o jogador local sem reduzir o tamanho dos personagens.
const SPAWNS=SPAWN_TEMPLATE.map(s=>({...s,x:s.x+CW}));
...
const BASE_PLATFORMS=Array.from({length:3},(_,seg)=>PLATFORM_TEMPLATE.map(p=>({...p,id:`${p.id}-${seg}`,x:p.x+seg*CW}))).flat();
function makeBridges(){return Array.from({length:3},(_,seg)=>[{id:`b1-${seg}`,x:480+seg*CW,y:600,w:100,h:18,hp:80,maxHp:80,brokenT:0},{id:`b2-${seg}`,x:860+seg*CW,y:600,w:100,h:18,hp:80,maxHp:80,brokenT:0}]).flat()}
...
let selectedFormat='ffa',selectedChar='',net=null,game=null,lastLobby=null,charCache=null,lobbyContext='island';
const DUO_EVENT_REWARD=Object.freeze({psy:170,gold:20000,vandais:500});
...
function allCharacters(){
  const out=[],seen=new Set(),add=c=>{if(!c?.id||c.ravamLegendExclusive||seen.has(c.id)||(c.id==='ztaaa'&&!(state?.ztaaaUnlocked&&state?.roster?.includes('ztaaa'))))return;seen.add(c.id);out.push(c)};
...
function injectModeCard(){
  ...
  function patched(){const r=base.apply(this,arguments);const grid=document.querySelector('.mode-grid');if(grid&&!document.getElementById('mode-island')){const b=document.createElement('button');b.id='mode-island';b.className='mode-card mk-mode island-mode-card';b.innerHTML='<div class="mode-icon">🏝️</div><span class="mode-badge">ONLINE P2P · ATÉ 8</span><h3>CONFRONTO <span>DA ILHA</span></h3><p>Platform fighter em arena 3× maior, com câmera dinâmica, pulo duplo e combate completo do modo normal.</p>';b.onclick=()=>{playSfx('menu_confirm');openSetup()};grid.appendChild(b)}if(grid&&!document.getElementById('mode-duo-event')){const e=document.createElement('button');e.id='mode-duo-event';e.className='mode-card mk-mode duo-event-mode-card';e.innerHTML='<div class="mode-icon">🤝</div><span class="mode-badge">EVENTO P2P · SOMENTE DUPLAS</span><h3>EVENTO <span>EM DUPLAS</span></h3><p>Vença 10 confrontos com seu parceiro. Duas derrotas eliminam a dupla. ZTAAA espera na batalha final.</p>';e.onclick=()=>{playSfx('menu_confirm');openDuoEvent()};grid.appendChild(e)}return r}
  patched.__islandV22=true;window.renderModes=patched;
}
function duoEventState(){
  if(!state.duoEventV228||typeof state.duoEventV228!=='object')state.duoEventV228={wins:0,losses:0,eliminated:false,completed:false};
  const st=state.duoEventV228;st.wins=clamp(st.wins||0,0,10);st.losses=clamp(st.losses||0,0,2);st.eliminated=!!st.eliminated;st.completed=!!st.completed;return st;
}
function syncDuoEventState(src){if(!src)return duoEventState();const st=duoEventState();st.wins=clamp(src.wins??st.wins,0,10);st.losses=clamp(src.losses??st.losses,0,2);st.eliminated=!!(src.eliminated??st.eliminated);st.completed=!!(src.completed??st.completed);try{persist()}catch(_){}return st}
...
function duoEventProgressHtml(st){return`<div class="duo-event-progress"><div><b>${st.wins}/10</b><span>VITÓRIAS</span></div><div><b>${st.losses}/2</b><span>DERROTAS</span></div><div><b>${Math.max(0,10-st.wins)}</b><span>FALTAM</span></div></div>`}
function openDuoEvent(){
  stopGame(false);closeNetwork();lobbyContext='duo';selectedFormat='duo';screen='island-duo-event';drawCanvas?.();const st=duoEventState();
  const status=st.completed?'EVENTO CONCLUÍDO':st.eliminated?'DUPLA ELIMINADA':'DESAFIO ATIVO';
  app.innerHTML=`<main class="card island-shell duo-event-shell"><div class="island-kicker">EVENTO OFICIAL · P2P</div><h1 class="island-title">EVENTO <span>EM DUPLAS</span></h1><p class="island-sub">Somente dois jogadores reais. Vencendo 9 confrontos, a 10ª partida é contra <b>ZTAAA</b>. Ao chegar a <b>2 derrotas</b>, o evento termina e não pode ser tentado novamente nesta instalação.</p>${duoEventProgressHtml(st)}<div class="duo-event-rules"><article><b>🤝 DUPLA P2P</b><span>HOST + CLIENT jogam sempre no mesmo time.</span></article><article><b>🏆 10 VITÓRIAS</b><span>A vitória final precisa ser contra ZTAAA.</span></article><article><b>☠️ 2 DERROTAS</b><span>Eliminação permanente do evento.</span></article><article><b>🎁 RECOMPENSA</b><span>170 PSY · 20.000 GOLD · 500 VANDAIS para cada jogador.</span></article></div><div class="duo-event-state ${st.completed?'complete':st.eliminated?'out':'active'}">${status}</div><div class="island-actions">${(!st.eliminated&&!st.completed)?'<button id="duo-event-host" class="btn big island-online-btn">🌐 CRIAR DUPLA P2P</button><button id="duo-event-join" class="btn big island-online-btn">🔗 ENTRAR NA DUPLA</button>':''}</div><div class="island-foot"><span>O progresso e as derrotas ficam salvos.</span><button id="duo-event-back" class="btn">← MODOS</button></div></main>`;
  document.getElementById('duo-event-host')?.addEventListener('click',()=>openDuoEventLobby('host'));document.getElementById('duo-event-join')?.addEventListener('click',()=>openDuoEventLobby('guest'));document.getElementById('duo-event-back').onclick=()=>{lobbyContext='island';selectedFormat='ffa';renderModes()};
}
function openDuoEventLobby(role){
  ...
  app.innerHTML=`<main class="card island-shell duo-event-shell"><div class="island-kicker">EVENTO EM DUPLAS · V${VERSION}</div><h1 class="island-title">DUPLA <span>P2P</span></h1>${duoEventProgressHtml(st)}<p class="island-sub">Escolha seu lutador, marque PRONTO e enfrente a próxima partida da sequência. Os dois humanos ficam no mesmo time.</p><div class="island-lobby"><section class="island-panel"><h3>${role==='host'?'CRIAR DUPLA':'ENTRAR NA DUPLA'}</h3><div id="island-connect-panel">${role==='host'?'<p style="color:#91a4ba;font:700 11px Oxanium">Gerando código exclusivo do evento…</p>':'<input id="island-room-input" class="island-room-code" maxlength="12" placeholder="DUPLA-ABC123"><button id="island-connect" class="btn big island-online-btn" style="width:100%;margin-top:10px">CONECTAR</button>'}</div><div id="island-net-status" class="island-lobby-status">${role==='host'?'Preparando a sala…':'Digite o código da dupla.'}</div><div class="island-lobby-pick-head"><b>1 · SEU LUTADOR</b><span id="island-lobby-char-name">${esc(charById(selectedChar).name||selectedChar)}</span></div><div id="island-lobby-roster" class="island-lobby-roster">${roster.map(c=>`<button class="island-lobby-fighter ${c.id===selectedChar?'active':''}" data-char="${esc(c.id)}" style="--fc:${c.color||'#67e8f9'}">${portraitOf(c)?`<img src="${esc(portraitOf(c))}" alt="">`:`<span>${esc((c.name||c.id).slice(0,2).toUpperCase())}</span>`}<b>${esc(c.name||c.id)}</b></button>`).join('')}</div><div class="island-lobby-ready-note">2 · Depois da escolha, marque <b>PRONTO</b>.</div></section><section class="island-panel"><div class="island-roster-head"><h3 id="island-lobby-title">EVENTO EM DUPLAS</h3><small id="island-capacity">0 / 2</small></div><div id="island-player-list" class="island-player-list"></div><div class="island-lobby-controls"><button id="island-ready" class="btn island-ready-toggle">✓ MARCAR COMO PRONTO</button><button id="island-start" class="btn big" ${role==='host'?'':'hidden'}>AGUARDANDO DUPLA</button><button id="island-copy" class="btn" hidden>COPIAR CÓDIGO</button></div></section></div><div class="island-foot"><span>2 JOGADORES · 10 VITÓRIAS · 2 DERROTAS = FORA</span><button id="island-lobby-back" class="btn">← SAIR</button></div></main>`;
...
function renderExistingDuoEventLobby(message='✓ Próxima partida liberada. Escolham os lutadores e marquem PRONTO.'){
  ...
  app.innerHTML=`<main class="card island-shell duo-event-shell"><div class="island-kicker">EVENTO EM DUPLAS · SALA PRESERVADA</div><h1 class="island-title">PRÓXIMA <span>BATALHA</span></h1>${duoEventProgressHtml(st)}<p class="island-sub">${esc(message)} O código continua <b>${esc(net.room||'')}</b>.</p><div class="island-lobby"><section class="island-panel"><h3>ESCOLHA DE PERSONAGEM</h3><div id="island-connect-panel"><input id="island-room-code" class="island-room-code" readonly value="${esc(net.room||'')}"></div><div id="island-net-status" class="island-lobby-status">✓ Conexão da dupla mantida.</div><div class="island-lobby-pick-head"><b>1 · SEU LUTADOR</b><span id="island-lobby-char-name">${esc(charById(selectedChar).name||selectedChar)}</span></div><div id="island-lobby-roster" class="island-lobby-roster">${roster.map(c=>`<button class="island-lobby-fighter ${c.id===selectedChar?'active':''}" data-char="${esc(c.id)}" style="--fc:${c.color||'#67e8f9'}">${portraitOf(c)?`<img src="${esc(portraitOf(c))}" alt="">`:`<span>${esc((c.name||c.id).slice(0,2).toUpperCase())}</span>`}<b>${esc(c.name||c.id)}</b></button>`).join('')}</div></section><section class="island-panel"><div class="island-roster-head"><h3 id="island-lobby-title">EVENTO EM DUPLAS</h3><small id="island-capacity">${net.players?.length||0} / 2</small></div><div id="island-player-list" class="island-player-list"></div><div class="island-lobby-controls"><button id="island-ready" class="btn island-ready-toggle">✓ MARCAR COMO PRONTO</button><button id="island-start" class="btn big" ${role==='host'?'':'hidden'}>AGUARDANDO DUPLA</button><button id="island-copy" class="btn">COPIAR CÓDIGO</button></div></section></div><div class="island-foot"><span>${st.wins>=9?'PRÓXIMO: ZTAAA · BOSS FINAL':'PRÓXIMO: CONFRONTO '+(st.wins+1)+'/10'}</span><button id="island-lobby-back" class="btn">← SAIR DO EVENTO</button></div></main>`;
...
function hostStartDuoEvent(){
  if(net?.role!=='host'||net.mode!=='duo')return;const st=duoEventState();if(st.eliminated||st.completed)return openDuoEvent();if(net.players.length!==2||!net.players.every(p=>p.ready)){netStatus('São necessários os 2 jogadores PRONTOS.');return}
  const humans=net.players.map((p,i)=>({...p,slot:i,team:0,bot:false,ready:true}));let desc,boss=st.wins>=9;
  if(boss){const z={id:'ztaaa',name:'ZTAAA',color:'#ff3030',dmg:85,speed:105,hp:999999};desc=[...humans,{id:'event-ztaaa',slot:2,name:'ZTAAA · BOSS FINAL',charId:'ztaaa',team:1,ready:true,bot:true,boss:true}]}else{const pool=duoEventOpponentPool(),a=pool[(st.wins*2)%pool.length],b=pool[(st.wins*2+1)%pool.length];desc=[...humans,{id:'event-bot-a',slot:2,name:a.name||'RIVAL A',charId:a.id,team:1,ready:true,bot:true},{id:'event-bot-b',slot:3,name:b.name||'RIVAL B',charId:b.id,team:1,ready:true,bot:true}]}
  const ev={wins:st.wins,losses:st.losses,boss,match:st.wins+1};broadcast({type:'start',mode:'2v2',players:desc,event:ev});startGameFromDescriptors(desc,'2v2',{authoritative:true,online:true,event:ev});
}
function showDuoEventConclusion(kind){
  stopGame(false);screen='island-duo-event-result';drawCanvas?.();const st=duoEventState(),complete=kind==='complete';app.innerHTML=`<main class="card island-shell duo-event-shell duo-event-final ${complete?'complete':'out'}"><div class="island-kicker">EVENTO EM DUPLAS</div><h1 class="island-title">${complete?'EVENTO <span>CONCLUÍDO</span>':'DUPLA <span>ELIMINADA</span>'}</h1>${duoEventProgressHtml(st)}<p class="island-sub">${complete?'Vocês venceram as 10 partidas e derrotaram ZTAAA. A premiação foi entregue para HOST e CLIENT.':'A dupla chegou a 2 derrotas. Este evento não pode mais ser tentado nesta instalação.'}</p>${complete?'<div class="duo-event-rewards"><b>💠 170 PSY</b><b>🪙 20.000 GOLD</b><b>🟪 500 VANDAIS</b></div>':''}<div class="island-actions"><button id="duo-final-modes" class="btn big">⚔️ VOLTAR AOS MODOS</button></div></main>`;document.getElementById('duo-final-modes').onclick=()=>{closeNetwork();lobbyContext='island';selectedFormat='ffa';renderModes()};
}
function handleDuoEventResult(){
  if(!game?.duoEvent||net?.role!=='host')return;const won=Number(game.winner)===0,st=duoEventState();if(won)st.wins=clamp(st.wins+1,0,10);else st.losses=clamp(st.losses+1,0,2);if(st.wins>=10)st.completed=true;if(st.losses>=2&&!st.completed)st.eliminated=true;try{persist()}catch(_){}
...
function openSetup(){
  stopGame(false);closeNetwork();lobbyContext='island';if(selectedFormat==='duo')selectedFormat='ffa';charCache=null;screen='island-select';drawCanvas?.();
  const roster=allCharacters();if(!roster.some(c=>c.id===selectedChar))selectedChar=roster[0]?.id||'rojo';
  app.innerHTML=`<main class="card island-shell"><div class="island-kicker">LUTADOR V22 · PLATFORM FIGHTER</div><h1 class="island-title">CONFRONTO <span>DA ILHA</span></h1><p class="island-sub">Uma ilha 3× maior para até 8 lutadores. A câmera acompanha a ação, o pulo continua sendo duplo e cada lutador preserva animações, golpes e poderes do combate normal.</p><div class="island-setup-grid"><section class="island-panel"><h3>1 · FORMATO DA BATALHA</h3><div class="island-format-grid">${Object.values(FORMAT).filter(f=>f.id!=='duo').map(f=>`<button class="island-format ${f.id===selectedFormat?'active':''}" data-format="${f.id}"><b>${f.short}</b>${f.name}</button>`).join('')}</div><div class="island-rules"><div class="island-rule"><i>💀</i><span><strong>3 vidas</strong><br>Quanto maior o dano, maior o lançamento.</span></div><div class="island-rule"><i>🌉</i><span><strong>Pontes quebráveis</strong><br>Elas voltam após 12 segundos.</span></div><div class="island-rule"><i>✚</i><span><strong>Suprimentos frequentes</strong><br>Vida e congelamento a cada 6–8s.</span></div><div class="island-rule"><i>⚡</i><span><strong>Combate normal preservado</strong><br>I/O/G, J, K, L, H e pulo duplo por personagem.</span></div></div></section><section class="island-panel"><div class="island-roster-head"><h3>2 · CAMPEÃO DESBLOQUEADO</h3><small>${roster.length} DISPONÍVEIS</small></div><div class="island-roster">${roster.map(c=>`<button class="island-fighter ${c.id===selectedChar?'active':''}" data-char="${esc(c.id)}" style="--fc:${c.color||'#67e8f9'}">${portraitOf(c)?`<img src="${esc(portraitOf(c))}" alt="">`:`<span class="island-face">${esc((c.name||c.id).slice(0,2).toUpperCase())}</span>`}<b>${esc(c.name||c.id)}</b><small>${esc(c.ravamOnly?'R.A.V.A.M':(['ktrak','laranja','jone'].includes(c.id)?'PRIME':'CAMPEÃO'))}</small></button>`).join('')}</div><div id="island-selected"></div></section></div><div class="island-actions"><button id="island-host" class="btn big island-online-btn">🌐 CRIAR SALA P2P</button><button id="island-join" class="btn big island-online-btn">🔗 ENTRAR POR CÓDIGO</button><button id="island-train" class="btn big island-training-btn">🤖 TREINAR COM BOTS</button></div><div class="island-foot"><span>CONTROLES · A/D mover · W pulo 2× · S descer · I soco · O chute · G gancho · J poder/técnica · K defesa · L super · H extra · Q esquiva · U/P extras especiais</span><button id="island-back" class="btn">← MODOS</button></div></main>`;
  ...
  document.getElementById('island-host').onclick=()=>openLobby('host');document.getElementById('island-join').onclick=()=>openLobby('guest');document.getElementById('island-train').onclick=startTraining;document.getElementById('island-back').onclick=()=>renderModes();updateSelected();
}
...
function openLobby(role){
  ...
  const roster=allCharacters();if(!roster.some(c=>c.id===selectedChar))selectedChar=roster[0]?.id||'rojo';
  app.innerHTML=`<main class="card island-shell"><div class="island-kicker">REDE WEBRTC · V${VERSION}</div><h1 class="island-title">SALA <span>P2P</span></h1><p class="island-sub">Escolha seu personagem dentro da sala e só depois marque PRONTO. Se trocar de personagem, seu estado volta automaticamente para ESCOLHENDO.</p><div class="island-lobby"><section class="island-panel"><h3>${role==='host'?'CRIAR SALA':'ENTRAR NA SALA'}</h3><div id="island-connect-panel">${role==='host'?'<p style="color:#91a4ba;font:700 11px Oxanium">Preparando um código seguro para até 8 jogadores…</p>':`<input id="island-room-input" class="island-room-code" maxlength="18" placeholder="ILHA-ABC123"><button id="island-connect" class="btn big island-online-btn" style="width:100%;margin-top:10px">CONECTAR</button>`}</div><div id="island-net-status" class="island-lobby-status">${role==='host'?'Carregando rede online…':'Escolha seu personagem e digite o código da sala.'}</div><div class="island-net-note">A conexão usa internet P2P/WebRTC. O formato escolhido pelo HOST define o tamanho da sala.</div><div class="island-lobby-pick-head"><b>1 · ESCOLHA SEU PERSONAGEM</b><span id="island-lobby-char-name">${esc(charById(selectedChar).name||selectedChar)}</span></div><div id="island-lobby-roster" class="island-lobby-roster">${roster.map(c=>`<button class="island-lobby-fighter ${c.id===selectedChar?'active':''}" data-char="${esc(c.id)}" style="--fc:${c.color||'#67e8f9'}">${portraitOf(c)?`<img src="${esc(portraitOf(c))}" alt="">`:`<span>${esc((c.name||c.id).slice(0,2).toUpperCase())}</span>`}<b>${esc(c.name||c.id)}</b></button>`).join('')}</div><div class="island-lobby-ready-note">2 · Depois de escolher, marque <b>PRONTO</b>.</div></section><section class="island-panel"><div class="island-roster-head"><h3 id="island-lobby-title">${fmtName(selectedFormat)}</h3><small id="island-capacity">0 / ${FORMAT[selectedFormat].max}</small></div><div id="island-player-list" class="island-player-list"><div class="island-player-row island-empty-slot"><span class="dot"></span><b>AGUARDANDO REDE…</b><em>—</em></div></div><div class="island-lobby-controls"><button id="island-ready" class="btn island-ready-toggle">✓ PRONTO</button><button id="island-start" class="btn big" hidden>⚔️ INICIAR BATALHA</button><button id="island-copy" class="btn" hidden>COPIAR CÓDIGO</button></div></section></div><div class="island-foot"><span id="island-lobby-help">ESCOLHA PERSONAGEM → PRONTO → HOST INICIA</span><button id="island-lobby-back" class="btn">← VOLTAR</button></div></main>`;
...
function roomCode(){return(lobbyContext==='duo'?'DUPLA-':'ILHA-')+Math.random().toString(36).slice(2,8).toUpperCase()}
...
function createHost(){ensurePeer(()=>{
  closeNetwork();const code=roomCode(),peer=new Peer(code,peerOptions());net={role:'host',peer,room:code,connections:new Map(),players:[basePlayer('host',0,playerName(),selectedChar,false)],localId:'host',ready:false,mode:selectedFormat,context:lobbyContext,eventState:lobbyContext==='duo'?{...duoEventState()}:null};
  peer.on('open',()=>{const p=document.getElementById('island-connect-panel');if(p)p.innerHTML=`<input id="island-room-code" class="island-room-code" readonly value="${code}"><p style="color:#91a4ba;font:700 10px/1.5 Oxanium">Envie este código aos jogadores. A sala comporta ${FORMAT[selectedFormat].max} participantes.</p>`;const copy=document.getElementById('island-copy');if(copy){copy.hidden=false;copy.onclick=()=>copyRoom(code)}const start=document.getElementById('island-start');if(start){start.hidden=false;start.onclick=hostStart}netStatus('✓ Sala criada. Aguardando jogadores…',true);renderLobbyState(net.players)});
...
function copyRoom(code){navigator.clipboard?.writeText(code).then(()=>netStatus('✓ Código copiado.',true)).catch(()=>netStatus('Código: '+code,true))}
...
function bindHostConnection(conn){
  ...
  conn.on('data',msg=>{if(!net||net.role!=='host'||!msg||typeof msg!=='object')return;entry.lastSeen=performance.now();
    if(msg.type==='hello'){
      ...
      if(net.context==='duo'&&(msg.eventState?.eliminated||msg.eventState?.completed)){conn.send({type:'error',message:msg.eventState?.eliminated?'Este jogador já foi eliminado do Evento em Duplas.':'Este jogador já concluiu o Evento em Duplas.'});conn.close();return}const cap=FORMAT[net.mode].max;if(net.players.length>=cap){conn.send({type:'error',message:'A sala já está cheia.'});conn.close();return}
      const id='p'+Math.random().toString(36).slice(2,8),slot=net.players.length,p=basePlayer(id,slot,msg.name,msg.charId,false);entry.playerId=id;net.connections.set(id,entry);net.players.push(p);conn.send({type:'welcome',id,mode:net.mode,version:VERSION,host:playerName(),context:net.context,eventState:net.context==='duo'?{...duoEventState()}:null});setTimeout(()=>openFastStateChannel(entry),80);broadcastLobby();netStatus(`✓ ${p.name} entrou na sala.`,true);return;
    }
  ...
  const leave=()=>{try{entry.fast?.close()}catch(_){} if(!net||net.role!=='host'||!entry.playerId)return;const p=net.players.find(x=>x.id===entry.playerId);net.connections.delete(entry.playerId);net.players=net.players.filter(x=>x.id!==entry.playerId);net.players.forEach((x,i)=>{x.slot=i;x.team=net.mode==='duo'?0:(net.mode==='ffa'?i:i%2)});if(game?.authoritative){const a=game.actors.find(x=>x.playerId===entry.playerId);if(a){a.stocks=0;a.eliminated=true}}broadcastLobby();netStatus(`⚠ ${p?.name||'Jogador'} saiu.`)};
...
function lobbyPacket(){return{type:'lobby',mode:net.mode,room:net.room,context:net.context||lobbyContext,eventState:net.mode==='duo'?{...duoEventState()}:null,players:net.players.map(p=>({...p}))}}
function broadcastLobby(){if(net?.role!=='host')return;const msg=lobbyPacket();broadcast(msg);renderLobbyState(msg.players)}
function connectGuest(){
  const code=String(document.getElementById('island-room-input')?.value||'').trim().toUpperCase();if(!/^(ILHA|DUPLA)-[A-Z0-9]{6}$/.test(code)){netStatus('✕ Código inválido. Ex.: ILHA-ABC123 ou DUPLA-ABC123');return}lobbyContext=code.startsWith('DUPLA-')?'duo':'island';if(lobbyContext==='duo')selectedFormat='duo';
...
function bindGuestConnection(conn){
  ...
  conn.on('data',msg=>{if(!net||net.role!=='guest'||!msg||typeof msg!=='object')return;
    ...
    if(msg.type==='welcome'){net.localId=msg.id;net.mode=msg.mode;net.context=msg.context||net.context;lobbyContext=net.context==='duo'?'duo':'island';selectedFormat=msg.mode;if(msg.eventState)syncDuoEventState(msg.eventState);netStatus(`✓ Conectado ao HOST ${msg.host}.`,true);return}
    if(msg.type==='lobby'){net.players=msg.players||[];net.mode=msg.mode;net.context=msg.context||net.context;if(msg.eventState)syncDuoEventState(msg.eventState);lastLobby=msg;renderLobbyState(net.players);return}
    if(msg.type==='start'){startGameFromDescriptors(msg.players,msg.mode,{authoritative:false,online:true,event:msg.event||null});return}
  ...
  conn.on('close',()=>{netStatus('⚠ Conexão encerrada pelo HOST.')});conn.on('error',()=>netStatus('✕ Erro na conexão P2P.'));
}
function renderLobbyState(players){
  if(!['island-lobby','island-duo-lobby'].includes(screen))return;const mode=net?.mode||selectedFormat,f=FORMAT[mode],list=document.getElementById('island-player-list'),cap=document.getElementById('island-capacity'),title=document.getElementById('island-lobby-title');if(title)title.textContent=f.name;if(cap)cap.textContent=`${players.length} / ${f.max}`;
  if(list){const rows=[];for(let i=0;i<f.max;i++){const p=players[i];if(p){const c=charById(p.charId),tc=TEAM_COLORS[p.team%TEAM_COLORS.length];rows.push(`<div class="island-player-row" style="--pc:${tc}"><span class="dot"></span><div><b>${esc(p.name)}${p.id===(net?.localId||'host')?' · VOCÊ':''}</b><span>${esc(c.name||p.charId)}</span></div><em>${mode==='duo'?'DUPLA':mode==='ffa'?`P${i+1}`:`TIME ${p.team+1}`} · ${p.ready?'PRONTO':'ESCOLHENDO'}</em></div>`)}else rows.push('<div class="island-player-row island-empty-slot" style="--pc:#475569"><span class="dot"></span><div><b>VAGA ABERTA</b><span>aguardando jogador</span></div><em>—</em></div>')}list.innerHTML=rows.join('')}
  ...
  const start=document.getElementById('island-start');if(start){const valid=players.length>=f.min&&players.length<=f.max&&players.every(p=>p.ready);start.disabled=!valid;start.textContent=valid?'⚔️ INICIAR BATALHA':`AGUARDANDO ${f.min} JOGADORES PRONTOS`}
}
...
function hostStart(){if(net?.role!=='host')return;if(net.mode==='duo')return hostStartDuoEvent();const f=FORMAT[net.mode];if(net.players.length<f.min||!net.players.every(p=>p.ready)){netStatus(`São necessários ${f.min} jogadores prontos.`);return}const desc=net.players.map((p,i)=>({...p,slot:i,team:net.mode==='ffa'?i:i%2}));broadcast({type:'start',mode:net.mode,players:desc});startGameFromDescriptors(desc,net.mode,{authoritative:true,online:true})}
function startTraining(){const f=FORMAT[selectedFormat],count=f.max,roster=allCharacters(),desc=[basePlayer('local',0,playerName(),selectedChar,true)];for(let i=1;i<count;i++){const pool=roster.filter(c=>c.id!==selectedChar||roster.length===1),c=pool[(i*7+Math.floor(Math.random()*pool.length))%Math.max(1,pool.length)]||charById('rojo');desc.push(basePlayer('bot'+i,i,`BOT ${i}`,c.id,true));desc[i].bot=true}startGameFromDescriptors(desc,selectedFormat,{authoritative:true,online:false})}
...
function startGameFromDescriptors(players,mode,opts){
  ...
  const localId=opts.online?(net?.localId||'host'):'local';game={canvas,ctx,wrap,mode,format:FORMAT[mode],duoEvent:opts.event||null,authoritative:!!opts.authoritative,online:!!opts.online,localId,actors:players.map((p,i)=>makeActor(p,i,localId)),bridges:makeBridges(),projectiles:[],areas:[],pickups:[],fx:[],netEvents:[],phase:'countdown',countdown:3.2,time:0,pickupT:3.8,last:performance.now(),lastNet:0,lastInputSend:0,lastInputMask:-1,lastHud:0,winner:null,banner:'PREPARE-SE',bannerT:2.3,shake:0,paused:false,raf:0,cameraX:CW,cameraTargetX:CW,quality:(opts.online&&!opts.authoritative)?'medium':'high',perfFrames:0,perfTime:0,fps:60,stablePerf:0,predPrevInput:{},snapshotSeq:0,lastSnapshotAt:0,netGap:0,inputSeq:0};
  buildHud();game.raf=requestAnimationFrame(gameLoop);playSfx('menu_confirm');
}
function makeActor(p,i,localId){
  ...
  return{playerId:p.id,name:p.name,charId:c.id,color:c.color||TEAM_COLORS[i],team:p.team,slot:i,local:p.id===localId,bot:!!p.bot,boss:!!p.boss,x:sp.x,y:sp.y-104,w:46,h:104,vx:0,vy:0,face:i%2?-1:1,onGround:false,jumps:2,dropT:0,damage:0,stocks:p.boss?4:3,meter:0,superHitsDealt:0,superHitsTaken:0,speed:baseSpeed,normalDmg:Math.round((c.dmg||30)*1.5*(p.boss?1.25:1)),attackCd:0,powerCd:0,specialCd:0,attackT:0,action:'idle',hurtT:0,invuln:1.8,dodgeCd:0,guard:false,freezeT:0,slowT:0,poisonT:0,poisonTick:.5,poisonKind:'',poisonMarks:0,burnT:0,burnTick:.45,eliminated:false,respawnT:0,netInput:{},prevInput:{},aiT:0,aiPlan:null,powerIndex:0,visual:null,lastHitBy:null,kos:0,deaths:0,proStamina:100,proMove:null,proDirHistory:[],proKnockdownT:0,proKnockdownLanded:false,ktrakElementIndex:0,ktrakElement:'water',ktrakAvatarT:0,ktrakAvatarIntroT:0,jonePoisonIndex:0,signature:null,powerLabel:'',ktrakHealT:0,ktrakHealTick:0,ktrakHealAmount:0,ktrakLightningChargeT:0,attackLockT:0,noJumpT:0,flightT:0,flightStrikeT:0,copiedCharId:null,copiedT:0,absorbT:0,lastDamageTaken:0,lastDamageDealt:0,kloppCopyLockT:0,kainFlightT:0,kainRepelT:0,kainControlSwapT:0,extra1Cd:0,extra2Cd:0}
}
...
function requestExit(){if(game?.online)return exitToModes();togglePause()}
...
function predictLocalActor(a,input,dt){
  ...
  if(input.jump&&!prev.jump&&(a.jumps??2)>0&&!a.guard&&(a.noJumpT||0)<=0){a.vy=-690;a.onGround=false;a.jumps=Math.max(0,(a.jumps??2)-1);a.action='air'}
  ...
  const prevBottom=a.y+(a.h||104);
  if((a.flightT||0)>0){a.flightT=Math.max(0,a.flightT-dt);a.vy+=(a.y<245?520:-620)*dt;a.vy=clamp(a.vy,-170,170)}else a.vy+=1840*dt;
  a.x=clamp(a.x+a.vx*dt,-80,WORLD_W+80);a.y+=a.vy*dt;a.onGround=false;
  if(a.vy>=0){let best=null;for(const p of activePlatforms()){const nb=a.y+(a.h||104);if(a.x+(a.w||46)*.38<p.x||a.x-(a.w||46)*.38>p.x+p.w)continue;if(prevBottom<=p.y+8&&nb>=p.y&&nb<=p.y+42){if(!best||p.y<best.y)best=p}}if(best){a.y=best.y-(a.h||104);a.vy=0;a.onGround=true;a.jumps=2;if(a.attackT<=0)a.action=Math.abs(a.vx)>20?'walk':'idle'}}
...
function updateGame(dt){const g=game;g.time+=dt;g.shake=Math.max(0,g.shake-dt*18);if(g.bannerT>0)g.bannerT-=dt;if(g.phase==='countdown'){g.countdown-=dt;if(g.countdown<=0){g.phase='fight';g.banner='LUTEM!';g.bannerT=1.2;playSfx('fight_start')}sendStateIfNeeded();return}if(g.phase!=='fight'){sendStateIfNeeded();return}
  ...
  g.pickupT-=dt;if(g.pickupT<=0){spawnPickup();g.pickupT=6+Math.random()*2}
...
function beginCombatMove(a,kind){
  const s=COMBAT_SPECS[kind];if(!s||a.attackCd>0||a.guard||a.freezeT>0||a.hurtT>0||a.proMove)return false;
  if((a.proStamina||0)<s.cost){addFx('meter',a.x,a.y+22,'#facc15',.2,24);return false}
  a.proStamina-=s.cost;a.attackCd=s.duration;a.attackT=s.duration;a.action=kind==='uppercut'?'hook':kind;a.proMove={kind,time:0,duration:s.duration,start:s.start,end:s.end,hit:false,dir:a.face||1};a.vx=0;return true;
}
function hitCombatMove(a,m){
  const s=COMBAT_SPECS[m.kind];if(!s)return;const projectileMove=['down','corewave','comet','astralrain'].includes(m.kind);
  if(projectileMove){
  ...
  let hit=false;for(const b of game.actors){
    ...
    const dmg=islandMoveDamage(a,s.damage)*(1+(m.chain||0)*.08);
...
function updateActor(a,dt){
  ...
  if(a.freezeT<=0&&a.hurtT<=0&&!a.signature){
    const move=(input.left?-1:0)+(input.right?1:0),accel=a.onGround?14:8,max=a.speed*(a.slowT>0?.62:1)*(a.guard?.35:1);if(!a.proMove)a.vx+=(move*max-a.vx)*Math.min(1,accel*dt);if(move)a.face=move;
    // PULO DUPLO: duas pressões de W antes de tocar novamente no chão.
    if(pressed(a,input,'jump')&&a.jumps>0&&!a.guard&&a.noJumpT<=0){a.vy=-690;a.onGround=false;a.jumps--;a.dropT=0;addFx('jump',a.x,a.y+a.h,'#e2e8f0',.22,28);playSfx('jump',.35)}
    if(input.down&&a.onGround&&!a.proMove){a.dropT=.22;a.onGround=false;a.y+=5}
    if(pressed(a,input,'dodge'))dodge(a);
    if(a.attackLockT<=0){
      if(pressed(a,input,'punch'))beginCombatMove(a,'punch');
      if(pressed(a,input,'kick'))beginCombatMove(a,'kick');
      if(pressed(a,input,'hook'))beginCombatMove(a,'uppercut');
      if(pressed(a,input,'extra'))extraAction(a);
      if(pressed(a,input,'extra1'))lkughIslandExtra(a,'stars');
      if(pressed(a,input,'extra2'))lkughIslandExtra(a,'blackhole');
      if(pressed(a,input,'power')){const tech=requestedTechnique(a,input);if(tech)beginCombatMove(a,tech);else powerAttack(a,false)}
      if(pressed(a,input,'super'))powerAttack(a,true);
    }
  }else if(!a.signature&&!a.proMove)a.vx*=.94;
  const prevBottom=a.y+a.h;if(a.kainFlightT>0){const lift=(input.jump?1:0)-(input.down?1:0);a.vy+=(lift*-205-a.vy)*Math.min(1,dt*8);a.vy=clamp(a.vy,-205,205)}else if(a.flightT>0){a.vy+=(a.y<245?520:-620)*dt;a.vy=clamp(a.vy,-170,170)}else a.vy+=1840*dt;a.x+=a.vx*dt;a.y+=a.vy*dt;a.onGround=false;
...
function joneIslandPower(a,superMove){
  ...
  if(superMove){a.meter=0;a.superHitsDealt=0;a.superHitsTaken=0;a.attackCd=1.05;a.attackT=.75;a.action='special';spawnProjectile(a,{speed:420,vy:-360,gravity:900,damage:0,knock:0,r:17,life:.72,color:'#86ff58',effect:'poison',kind:'joneBomb',label:'PRAGA TÓXICA',explode:{rx:280,ry:135,life:7.4,damage:Math.max(1,(a.normalDmg||45)*.34*.10),knock:0,effect:'jonePlague',color:'#86ff58',tick:.27}});game.banner='JONE · PRAGA TÓXICA SUPREMA';game.bannerT=1;playSfx('super',.75);return}
  const order=['corrosive','acid','deep'],kind=order[a.jonePoisonIndex++%order.length],cfg={corrosive:['#72e85a',1.00],acid:['#c7f34b',1.08],deep:['#52e6ad',.88]}[kind],impact=Math.max(5.5,(a.normalDmg||45)*cfg[1]*.20);a.attackCd=.48;a.attackT=.35;a.action='special';spawnProjectile(a,{speed:650,damage:impact,knock:290,r:13,life:1.35,color:cfg[0],effect:'jone:'+kind,kind:'poison',label:kind.toUpperCase()});a.powerLabel=kind.toUpperCase();playSfx('special',.5)
}
...
function damageBridges(x,y,amount,range=50){for(const b of game.bridges){if(b.brokenT>0)continue;if(Math.abs(x-(b.x+b.w/2))<b.w/2+range&&Math.abs(y-b.y)<80){b.hp-=amount;if(b.hp<=0){b.hp=0;b.brokenT=12;game.banner='PONTE DESTRUÍDA';game.bannerT=.7;game.shake=9;addFx('bridge',b.x+b.w/2,b.y,'#f97316',.8,115);playSfx('hit_heavy',.65)}}}}
...
function spawnPickup(){const pads=activePlatforms().filter(p=>!p.id.startsWith('b')),p=pads[Math.floor(Math.random()*pads.length)],type=Math.random()<.63?'health':'freeze';game.pickups.push({id:Math.random().toString(36).slice(2),type,x:p.x+55+Math.random()*Math.max(20,p.w-110),y:p.y-36,life:18,bob:Math.random()*6});game.banner=type==='health'?'✚ VIDA CAIU NA ILHA':'❄ CONGELAMENTO DISPONÍVEL';game.bannerT=.65}
...
function loseStock(a){if(a.respawnT>0||a.eliminated)return;a.stocks--;a.deaths++;const killer=game.actors.find(x=>x.playerId===a.lastHitBy);if(killer&&killer!==a)killer.kos++;game.shake=12;addFx('ko',clamp(a.x,0,WORLD_W),clamp(a.y,0,CH),'#fff',.8,120);playSfx('ko',.7);if(a.stocks<=0){a.eliminated=true;a.x=-999;a.y=-999;game.banner=`${a.name.toUpperCase()} ELIMINADO`;game.bannerT=.8}else{a.respawnT=1.45;a.x=-999;a.y=-999}}
function respawn(a){const s=SPAWNS[a.slot%SPAWNS.length];a.x=s.x;a.y=s.y-104;a.vx=0;a.vy=0;a.damage=0;a.invuln=2;a.freezeT=0;a.poisonT=0;a.poisonMarks=0;a.burnT=0;a.jumps=2;a.onGround=false;a.lastHitBy=null;a.proMove=null;a.signature=null;a.ktrakHealT=0;a.ktrakLightningChargeT=0;a.attackLockT=0;a.noJumpT=0;a.flightT=0;a.absorbT=0;a.copiedT=0;a.copiedCharId=null;addFx('respawn',a.x,a.y+55,a.color,.8,90)}
...
function showResults(label){
  if(!game)return;const el=document.getElementById('island-results');if(el)el.classList.add('show');const w=document.getElementById('island-winner');if(w)w.textContent=`${label} VENCEU`;const copy=document.getElementById('island-result-copy');if(copy){const ranking=game.actors.slice().sort((a,b)=>b.stocks-a.stocks||b.kos-a.kos).map((a,i)=>`${i+1}º ${a.name} · ${a.kos} KOs · ${Math.max(0,a.stocks)} vidas`).join('<br>');copy.innerHTML=ranking+(game.duoEvent?'<br><br><b>Atualizando o progresso do Evento em Duplas…</b>':game.online?'<br><br><b>Voltando para a mesma sala…</b>':'')}
  playSfx('victory',.75);
...
function botInput(a,dt){a.aiT-=dt;if(a.aiT<=0){a.aiT=.08+Math.random()*.08;const enemies=game.actors.filter(b=>validTarget(a,b));let target=null;if(enemies.length){const allies=game.actors.filter(b=>b!==a&&!b.eliminated&&b.team===a.team),claimed=new Set(allies.map(x=>x.aiPlan?.target).filter(Boolean)),free=enemies.filter(e=>!claimed.has(e.playerId));target=(free.length?free:enemies).sort((x,y)=>Math.hypot(x.x-a.x,x.y-a.y)-Math.hypot(y.x-a.x,y.y-a.y))[0]}const ally=game.actors.find(b=>b!==a&&!b.eliminated&&b.team===a.team&&Math.abs(b.x-a.x)<85),dx=(target?.x||WORLD_W/2)-a.x,dy=(target?.y||a.y)-a.y,d=Math.abs(dx);a.aiPlan={target:target?.playerId,left:dx<-28,right:dx>28,jump:dy<-90||(!platformAhead(a,Math.sign(dx))&&d>100),down:dy>115,guard:!!target&&d<120&&target.attackT>0&&Math.random()<.55,punch:!!target&&d<102&&Math.abs(dy)<90&&Math.random()<.48,kick:!!target&&d<144&&Math.abs(dy)<90&&Math.random()<.30,hook:!!target&&d<126&&Math.abs(dy)<95&&Math.random()<.16,power:!!target&&d>90&&d<620&&Math.random()<.22,super:a.meter>=100&&!!target&&d<700,extra:a.charId==='ktrak'&&Math.random()<.035,dodge:!!target&&target.attackT>0&&d<150&&Math.random()<.32};if(ally){a.aiPlan.left=a.x>ally.x;a.aiPlan.right=a.x<=ally.x}if(a.x<65){a.aiPlan.left=false;a.aiPlan.right=true;a.aiPlan.jump=true}if(a.x>WORLD_W-65){a.aiPlan.right=false;a.aiPlan.left=true;a.aiPlan.jump=true}}
...
function makeSnapshot(centerX,seq){
  ...
  return{m:game.mode,p:game.phase,t:Math.round(game.time*100)/100,u:Math.round(game.pickupT*10)/10,b:game.banner,bt:Math.round(game.bannerT*10)/10,w:typeof game.winner==='object'?game.winner?.name:game.winner,n:seq,
...
injectModeCard();window.IslandClashV22=Object.freeze({version:VERSION,open:openSetup,openDuoEvent,stop:stopGame,formats:FORMAT,characters:allCharacters});
})();
