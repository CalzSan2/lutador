...
(()=>{
'use strict';
const VERSION='50.0.0',CW=1440,CH=810,WORLD_W=CW,FALL_Y=930;
...
const NORMAL_POWER_CD=Object.freeze({verry:.75,perry:.85,uiye:.65,ouip:.80,lkugh:.90,knunka:.90,dart:.65,jimmy:.70,hock:.72,vlad:.75,flame:.75,atizz:.42,klo:.85,nine85:.70,jone:.48,laranja:.46,ktrak:.52,charl:.62});
const NORMAL_SUPER_CD=Object.freeze({jimmy:6,hock:6.2,vlad:5.2,perry:4.8,verry:5.2,ouip:5,lkugh:5,dart:5.5,knunka:5.8,flame:5.5,klo:5.2,atizz:3.8,yoi:3.6,rtess:3.2,nine85:4.5,jone:6.6,laranja:5.8,ktrak:7.2,charl:8.5});
function normalPowerCooldown(a,id=a?.charId){
  if(id==='charl'&&(a.charlitoT||0)>0)return .48;
  if(id==='ktrak'){
...
const PROFILE={
  charl:['SUCATA TÁTICA','shot','#56dfff','metal'],
...
function injectModeCard(){
  ...
  function withSoloLabel(){const r=patched.apply(this,arguments),card=document.getElementById('mode-arena-total');if(card){const badge=card.querySelector('.mode-badge'),copy=card.querySelector('p');if(badge)badge.textContent='SOLO COM CPUs OU P2P';if(copy)copy.textContent='Jogue solo contra CPUs ou online em todos contra todos e equipes 2×2, 3×3 e 4×4.'}return r}
  withSoloLabel.__islandV22=true;window.renderModes=withSoloLabel;
}
...
function openSetup(){
  ...
  app.querySelector('.island-kicker').textContent=`ARENA TOTAL · SOLO OU P2P · V${VERSION}`;
  app.querySelector('.island-sub').textContent='Jogue solo contra CPUs sem criar sala ou escolha o P2P. Todos contra todos e equipes usam os mesmos golpes, poderes, supers e animações.';
  app.querySelector('.arena-total-actions')?.insertAdjacentHTML('afterbegin','<button id="island-solo" class="btn big island-solo-btn">🤖 JOGAR SOLO VS CPUs</button>');
  app.querySelector('.arena-total-actions')?.insertAdjacentHTML('afterend','<p class="arena-rule-preview">No solo, as vagas restantes são preenchidas por CPUs. Ajuste o número de participantes do livre-para-todos nas regras abaixo; equipes usam 2×2, 3×3 ou 4×4.</p>');
  document.getElementById('island-solo').onclick=startSoloArena;
...
function resetRoomReady(){
  if(!net)return;
  net.ready=false;
  for(const p of net.players||[])p.ready=!!p.bot;
}
...
function addLobbyBot(){
  if(net?.role!=='host'||net.mode==='duo'||game?.online)return;
  const max=roomCapacity(net.mode,net.rules);if(net.players.length>=max)return netStatus('Sala cheia. Remova alguém antes de adicionar CPU.');
  const pool=(typeof CHARACTERS!=='undefined'?CHARACTERS:[]).filter(c=>c?.id&&!c.codeOnly&&c.id!=='ztaaa');
  if(!pool.length)return netStatus('Nenhum campeão disponível para CPU.');
  const used=new Set(net.players.map(p=>p.charId)),choices=pool.filter(c=>!used.has(c.id)),pick=(choices.length?choices:pool)[Math.floor(Math.random()*(choices.length||pool.length))];
  const slot=net.players.length,counts=[0,1].map(t=>net.players.filter(p=>p.team===t).length),team=net.mode==='ffa'?slot:counts[0]<=counts[1]?0:1;
  net.players.push({id:'bot-'+Math.random().toString(36).slice(2,9),slot,name:'CPU · '+pick.name,charId:pick.id,ready:true,team,ping:0,bot:true});
  broadcastLobby();netStatus(`✓ CPU ${pick.name} adicionada ao ${net.mode==='ffa'?'confronto':'time '+(team+1)}.`,true);
}
function removeLobbyBot(){
  if(net?.role!=='host'||net.mode==='duo'||game?.online)return;
  let index=-1;for(let i=net.players.length-1;i>=0;i--)if(net.players[i].bot){index=i;break}if(index<0)return;
  net.players.splice(index,1);net.players.forEach((p,i)=>{p.slot=i;if(net.mode==='ffa')p.team=i});broadcastLobby();netStatus('✓ Última CPU removida.',true);
}
...
function lobbyPacket(){net.players.forEach((p,i)=>{p.slot=i;if(net.mode==='ffa')p.team=i});return{type:'lobby',mode:net.mode,room:net.room,context:net.context||lobbyContext,rules:net.rules,eventState:net.mode==='duo'?{...duoEventState()}:null,players:net.players.map(p=>({...p}))}}
...
function bindGuestConnection(conn){
  ...
  conn.on('data',msg=>{if(!net||net.role!=='guest'||!msg||typeof msg!=='object')return;
    ...
    if(msg.type==='room-return'){net.mode=msg.mode||net.mode;net.room=msg.room||net.room;net.players=(msg.players||net.players||[]).map(p=>({...p,ready:!!p.bot}));net.ready=false;returnOnlineMatchToRoom();return}
...
function renderLobbyState(players){
  ...
  let botControls=document.getElementById('island-bot-controls');if(arenaTotalActive&&mode!=='duo'&&net?.role==='host'&&list){if(!botControls){list.insertAdjacentHTML('afterend','<div id="island-bot-controls" class="island-lobby-controls"><button id="island-add-bot" class="btn">＋ ADICIONAR CPU</button><button id="island-remove-bot" class="btn">－ REMOVER CPU</button></div>');botControls=document.getElementById('island-bot-controls');document.getElementById('island-add-bot').onclick=addLobbyBot;document.getElementById('island-remove-bot').onclick=removeLobbyBot}document.getElementById('island-add-bot').disabled=players.length>=max;document.getElementById('island-remove-bot').disabled=!players.some(p=>p.bot)}else botControls?.remove();
let teamControls=document.getElementById('island-team-controls');if(mode!=='ffa'&&mode!=='duo'){if(!teamControls&&list){list.insertAdjacentHTML('afterend','<div id="island-team-controls" class="island-team-controls"><button id="island-change-team" class="btn">⇄ TROCAR DE TIME</button><small id="island-team-count"></small></div>');teamControls=document.getElementById('island-team-controls');document.getElementById('island-change-team').onclick=requestTeamChange}const counts=[0,1].map(t=>players.filter(p=>p.team===t).length),counter=document.getElementById('island-team-count');if(counter)counter.textContent=`AZUL ${counts[0]}/${f.max/2} · VERMELHO ${counts[1]}/${f.max/2}`;const me=players.find(p=>p.id===(net?.localId||'host')),button=document.getElementById('island-change-team');if(button){button.disabled=!me||(counts[1-me.team]>=f.max/2&&!players.some(p=>p.bot&&p.team===1-me.team));button.textContent=me?`⇄ TROCAR PARA O TIME ${me.team===0?'VERMELHO':'AZUL'}`:'⇄ TROCAR DE TIME'}}else teamControls?.remove();
  ...
  const start=document.getElementById('island-start');if(start){const balanced=mode==='ffa'||mode==='duo'||([0,1].every(t=>players.filter(p=>p.team===t).length===f.max/2)),valid=players.length>=f.min&&players.length<=max&&balanced&&players.every(p=>p.ready);start.disabled=!valid;start.textContent=valid?'⚔️ INICIAR BATALHA':!balanced?'EQUILIBRE OS TIMES':`AGUARDANDO ${f.min} PARTICIPANTES (JOGADORES OU CPUs)`}
}
function changeTeam(playerId){if(net?.role!=='host'||!FORMAT[net.mode]||['ffa','duo'].includes(net.mode))return;const p=net.players.find(x=>x.id===playerId);if(!p||p.bot)return;const next=1-p.team,capacity=FORMAT[net.mode].max/2;if(net.players.filter(x=>x.team===next).length>=capacity){const swap=net.players.find(x=>x.bot&&x.team===next);if(!swap){netStatus('Este time já está completo.');return}swap.team=p.team}p.team=next;p.ready=false;broadcastLobby()}
...
function startSoloArena(){
  const mode=FORMAT[selectedFormat]&&selectedFormat!=='duo'?selectedFormat:'ffa',rules=normalizeRules(selectedRules),count=mode==='ffa'?rules.capacity:FORMAT[mode].max;
  const owned=allCharacters();if(!owned.some(c=>c.id===selectedChar)){toast('Escolha um campeão desbloqueado antes de jogar.');return}
  closeNetwork();arenaTotalActive=true;lobbyContext='arena-total';
  const pool=(typeof CHARACTERS!=='undefined'?CHARACTERS:[]).filter(c=>c?.id&&!c.codeOnly&&!c.ravamOnly&&c.id!=='ztaaa');
  if(!pool.length){toast('Nenhum campeão disponível para as CPUs.');return}
  const used=new Set([selectedChar]),players=[{id:'local',slot:0,name:playerName(),charId:selectedChar,team:0,ready:true,bot:false}];
  for(let i=1;i<count;i++){const choices=pool.filter(c=>!used.has(c.id)),pick=(choices.length?choices:pool)[Math.floor(Math.random()*(choices.length||pool.length))];used.add(pick.id);players.push({id:`solo-bot-${i}`,slot:i,name:`CPU · ${pick.name}`,charId:pick.id,team:mode==='ffa'?i:i%2,ready:true,bot:true})}
  const variant=rules.theme==='random'?['aurora','tempestade','crepusculo'][Math.floor(Math.random()*3)]:rules.theme;
  startGameFromDescriptors(players,mode,{authoritative:true,online:false,variant,rules});
}
...
function startGameFromDescriptors(players,mode,opts){
  ...
  if(game.arenaTotal&&!game.duoEvent)for(const actor of game.actors)if(actor.bot)actor.stocks=rules.stocks;
  buildHud();buildArenaHudExtras();if(!opts.online&&arenaTotalActive){const live=game.hud?.querySelector('.island-topbar .live'),rematch=document.getElementById('island-rematch'),badge=document.getElementById('island-net-badge');if(live)live.textContent='SOLO VS CPUs';if(badge)badge.textContent='PARTIDA LOCAL';if(rematch){rematch.textContent='↻ JOGAR NOVAMENTE';rematch.onclick=startSoloArena}}game.raf=requestAnimationFrame(gameLoop);playSfx('menu_confirm');
}
...
function tickStatus(a,dt){
  if(a.charId==='charl'&&a.charlitoT>0)a.charlitoT=Math.max(0,a.charlitoT-dt);
...
function powerAttack(a,superMove){
  ...
  if(a.charId==='charl')return charlIslandPower(a,superMove);
...
function charlIslandPower(a,superMove){
  if(superMove){a.meter=0;a.superHitsDealt=0;a.superHitsTaken=0;a.charlitoT=6;a.attackCd=.62;a.attackT=.62;a.action='special';a.powerLabel='CHARLITO · 6s';game.banner='CHARL → CHARLITO · MEGATRON';game.bannerT=1.1;addFx('meter',a.x,a.y+36,'#56dfff',.75,108);playSfx('super',.7);return true}
  const giant=(a.charlitoT||0)>0;a.attackCd=giant?.49:.62;a.attackT=giant?.42:.38;a.action='special';a.powerLabel=giant?'CHARLITO · FOGUETE':'CHARL · PARAFUSOS E PLACAS';
  if(giant){spawnProjectile(a,{x:a.x+a.face*40,y:a.y+32,speed:790,damage:15,knock:570,r:21,life:1.55,color:'#ffac61',effect:'burn',kind:'rocket',label:'FOGUETE CHARLITO',explode:{rx:90,ry:78,life:.18,damage:4,knock:180,effect:'burn',color:'#ff9855',tick:.12}});addFx('dash',a.x+a.face*48,a.y+32,'#ff9855',.25,45,a.face)}
  else{spawnProjectile(a,{x:a.x+a.face*32,y:a.y+21,speed:890,damage:5.8,knock:230,r:8,life:1.3,color:'#e2e8f0',effect:'metal',kind:'screw',label:'PARAFUSO'});spawnProjectile(a,{x:a.x+a.face*30,y:a.y+52,speed:650,damage:7.8,knock:310,r:17,life:1.45,color:'#80dfff',effect:'metal',kind:'plate',label:'PLACA METÁLICA'})}
  playSfx('special',.52);return true;
}
...
function variantPowerAttack(a,variant){
  ...
  if(a.charId==='charl'&&(a.charlitoT||0)>0){a.powerCd=.55;a.face=nearestEnemy(a)?Math.sign(nearestEnemy(a).x-a.x)||a.face:a.face;charlIslandPower(a,false);return true}
...
function botInput(a,dt){a.aiT-=dt;if(a.aiT<=0){a.aiT=.025+Math.random()*.035;const enemies=game.actors.filter(b=>validTarget(a,b));let target=null;if(enemies.length){const allies=game.actors.filter(b=>b!==a&&!b.eliminated&&b.team===a.team),claimed=new Set(allies.map(x=>x.aiPlan?.target).filter(Boolean)),free=enemies.filter(e=>!claimed.has(e.playerId));target=(free.length?free:enemies).sort((x,y)=>Math.hypot(x.x-a.x,x.y-a.y)-Math.hypot(y.x-a.x,y.y-a.y))[0]}const ally=game.actors.find(b=>b!==a&&!b.eliminated&&b.team===a.team&&Math.abs(b.x-a.x)<62),dx=(target?.x||WORLD_W/2)-a.x,dy=(target?.y||a.y)-a.y,d=Math.abs(dx),incoming=!!target&&(target.attackT>0||target.proMove);const wantsBackstab=!!target&&!incoming&&d<240&&Math.random()<.34,flankX=target?(target.x-(target.face||1)*72):WORLD_W/2,moveDx=wantsBackstab?flankX-a.x:dx;a.aiPlan={target:target?.playerId,left:moveDx<-12,right:moveDx>12,jump:dy<-45||(dy>70&&canDropThrough(a))||(!platformAhead(a,Math.sign(moveDx))&&d>60),down:dy>70,guard:incoming&&d<190&&Math.random()<.94,punch:!!target&&d<112&&Math.abs(dy)<75&&Math.random()<.78,kick:!!target&&d<158&&Math.abs(dy)<85&&Math.random()<.64,hook:!!target&&d<126&&Math.abs(dy)<80&&Math.random()<.38,power:!!target&&d<720&&Math.random()<.46,super:a.meter>=100&&!!target&&d<760,extra:['ktrak','jone'].includes(a.charId)&&Math.random()<.11,dodge:incoming&&d<230&&Math.random()<.82};if(ally){a.aiPlan.left=a.x>ally.x;a.aiPlan.right=a.x<=ally.x}if(a.x<48){a.aiPlan.left=false;a.aiPlan.right=true;a.aiPlan.jump=true}if(a.x>WORLD_W-48){a.aiPlan.right=false;a.aiPlan.left=true;a.aiPlan.jump=true}}
...
function drawProjectile(c,q){
  ...
  if(['screw','plate','rocket'].includes(q.kind)){
    c.rotate(q.kind==='rocket'?0:game.time*(q.kind==='plate'?10:17));
    if(q.kind==='screw'){c.fillStyle='#d9e5ef';c.fillRect(-q.r,-3,q.r*2,6);c.fillRect(q.r*.45,-7,4,14)}
    else if(q.kind==='plate'){c.fillStyle='#849cad';c.strokeStyle='#56dfff';c.lineWidth=3;c.beginPath();for(let i=0;i<6;i++){const a=i*Math.PI/3,x=Math.cos(a)*q.r,y=Math.sin(a)*q.r;i?c.lineTo(x,y):c.moveTo(x,y)}c.closePath();c.fill();c.stroke()}
    else{c.scale(q.vx<0?-1:1,1);c.fillStyle='#8ea3b5';c.fillRect(-q.r,-8,q.r*2,16);c.fillStyle='#ffad5a';c.beginPath();c.moveTo(-q.r,-7);c.lineTo(-q.r-19,0);c.lineTo(-q.r,7);c.fill();c.fillStyle='#56dfff';c.fillRect(q.r*.15,-6,8,12)}
  }else if(q.kind==='lightningBeam'){
...
function privatePowerInfo(a){
  ...
  if(a.charId==='charl')return a.charlitoT>0?`CHARLITO · ${a.charlitoT.toFixed(1)}s · J FOGUETES`:'J PARAFUSOS/PLACAS · L CHARLITO';
  if(a.charId==='kain'){
